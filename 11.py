...
class Yarn(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        enable_pitch: bool = False,         # pitch_oct と pitch_pc をまとめて制御
        enable_ivl_same: bool = False,
        enable_ivl_prev: bool = False,
    ...
    def _build_pitch_cache(
        ...
        *,
        ...
        if int(self.pairs_pitch_oct) > 0:
            theta_oct = octave_id.to(torch.float32).unsqueeze(1) * inv.unsqueeze(0)
            ...
            cos[:, cols_oct] = cos_oct[:, cols_oct]
            sin[:, cols_oct] = sin_oct[:, cols_oct]

        if int(self.pairs_pitch_pc) > 0:
            theta_pc = pc_id.to(torch.float32).unsqueeze(1) * inv.unsqueeze(0)
            cos_pc = theta_pc.cos()
            sin_pc = theta_pc.sin()
            cols_pc = self._axis_cols(self.AXIS_PITCH_PC, int(self.pairs_pitch_pc), device=device)
            cos[:, cols_pc] = cos_pc[:, cols_pc]
            sin[:, cols_pc] = sin_pc[:, cols_pc]
