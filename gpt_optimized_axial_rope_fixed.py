...
class REMIPosPitchSinusoidalPE(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        pos_update_val = (token_ids - self.pos_start + self.pos_idx_offset).masked_fill(~pos_tok, 0)  # Bar/Docは0
        ...
        if self.carry_pitch:
            ...
            pitch_update_val = (token_ids - self.pitch_start + self.pitch_idx_offset).masked_fill(~pitch_tok, 0)
            pitch_idx = forward_fill_from_updates(pitch_update_val, pitch_update_mask)
        else:
            pitch_idx = (token_ids - self.pitch_start + self.pitch_idx_offset).masked_fill(~pitch_tok, 0)
...
class AxialRotaryEmbedding4D(nn.Module):
    ...
    def _maybe_init_caches(self, *, device: torch.device, seq_len: int) -> None:
        ...
        if (
            ...
            self._cos_pos, self._sin_pos = self._build_cache(self.theta_pos, self.pos_cache_len, device=device)
        ...
        if (
            ...
            self._cos_pitch, self._sin_pitch = self._build_cache(self.theta_pitch, self.pitch_cache_len, device=device)
