...
class DistributedDataLoader:
    ...
    - align_to_bos=True:
        Each returned sequence starts at BOS_ID. A batch is built by concatenating whole
        BOS-delimited spans (truncated per-span to max_seq_len).
    - align_to_bos=False:
        Falls back to the original contiguous slicing scheme.

    The public API matches the original loader: reset(), next_batch(), plus ntok_total/files.
    """
    ...
    def next_batch(self):
        ...
        if self.align_to_bos:
            # Build a BOS-aligned buffer of exactly (T + 1) tokens for this rank.
            while True:
                try:
            ...
            for s, e in zip(starts, ends):
                ...
                dst += seg_len
            if dst != self.T + 1:
                raise RuntimeError(f"BOS-aligned batch construction failed: expected {self.T + 1} tokens, got {dst}")
