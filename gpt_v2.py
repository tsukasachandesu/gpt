...
class AxialRotaryEmbedding4D(nn.Module):
    """4軸（index / bar / pos / pitch）の **MRoPE**（Qwen3-VL の Interleaved-MRoPE 互換）を生成する.

    重要な違い（元の axial RoPE 方式との比較）:
      * **周波数表（inv_freq）は head_dim 全体で 1 本**（shape=(head_dim/2,)）を共有し、
        各軸は **position id だけ**を変えて cos/sin を作る。
      * `pair_layout="block"` は Qwen 系の **chunked MRoPE**（[index... bar... pos... pitch...]）に相当。
      * `pair_layout="interleave"` は Qwen3-VL の **Interleaved-MRoPE** と同じ考え方で、
        base 軸（ここでは index）を基底にして、他軸の周波数インデックスを
        周期 `num_axes` のストライドで上書きする。

    参考（Qwen3-VL / vLLM 実装）:
      * 3D（T/H/W）版は `apply_interleaved_rope` として公開されており、
        `x_t[..., 1: mrope_section[1]*3 : 3] = x[1, ..., 1: mrope_section[1]*3 : 3]` のように
        **固定ストライドで上書き**するのが本質。\
        （本クラスはそれを 4 軸へ自然拡張）

    注意:
      * Interleaved の固定ストライド方式では、base 以外の各軸 i に対して
        `i + num_axes*(pairs_i-1) < total_pairs` が必要です。
        そうでない比率（例: 4軸で 2軸目に total_pairs/4 を大幅に超える割当）は
        仕様上表現できないためエラーにします。
    """
    ...
    def __init__(
        ...
        *,
        ...
        # ペア（head_dim/2）の並べ方: "block"（chunked MRoPE） or "interleave"（Interleaved-MRoPE）
        ...
        # MRoPE sections (sum must equal total_pairs)
        self.mrope_section = (self.pairs_index, self.pairs_bar, self.pairs_pos, self.pairs_pitch)
        if sum(self.mrope_section) != total_pairs:
            raise RuntimeError(f"Internal error: mrope_section sums to {sum(self.mrope_section)} != {total_pairs}")

        # Shared inverse frequencies for full head_dim.
        # NOTE: Qwen3-VL 系は head_dim 全体（rotary_dim=head_dim）で inv_freq を作り、
        # それを T/H/W の position_ids に掛ける（=軸ごとに position のみ変える）。
        theta_full = self._build_theta(self.head_dim, base=self.base)  # (total_pairs,)

        # (Optional) per-axis scaling. If you want strict Qwen互換にするなら apply_axes を揃える。
        theta_index = self._apply_rope_scaling(theta_full, apply=self.rope_scaling_apply_axes[0])
        theta_bar = self._apply_rope_scaling(theta_full, apply=self.rope_scaling_apply_axes[1])
        theta_pos = self._apply_rope_scaling(theta_full, apply=self.rope_scaling_apply_axes[2])
        theta_pitch = self._apply_rope_scaling(theta_full, apply=self.rope_scaling_apply_axes[3])
        ...
        # Validate interleaved layout feasibility (fixed-stride).
        if self.pair_layout == "interleave":
            ...
            for axis_id, sec in enumerate(self.mrope_section):
                ...
                if max_idx >= total_pairs:
                    raise ValueError(
                        "pair_layout='interleave' (fixed-stride) cannot represent this allocation: "
                        f"axis={axis_id} sec={sec}, total_pairs={total_pairs} -> max_idx={max_idx} out of range. "
    ...
    def _build_theta(dim: int, *, base: int) -> torch.Tensor:
        ...
        Args:
            dim: rotary dimension (= head_dim). Must be even.

        Returns:
            inv_freq with shape (dim/2,)
        """
        if dim % 2 != 0:
            raise ValueError(f"dim must be even, got {dim}")
        ar = torch.arange(0, dim, 2, dtype=torch.float32)
    ...
    @staticmethod
    def _combine_block(x_axes: torch.Tensor, mrope_section: tuple[int, int, int, int]) -> torch.Tensor:
        """Chunked MRoPE: [axis0... axis1... axis2... axis3...]"""
        # x_axes: (4, B, T, total_pairs)
        splits = x_axes.split(mrope_section, dim=-1)
        return torch.cat([seg[i] for i, seg in enumerate(splits)], dim=-1)

    @staticmethod
    def _combine_interleave_fixed_stride(x_axes: torch.Tensor, mrope_section: tuple[int, int, int, int]) -> torch.Tensor:
        """Interleaved-MRoPE（固定ストライド）.

        base 軸（0）を基底にして、axis i の周波数インデックス {i, i+N, i+2N, ...} を上書きする。
        """
        # x_axes: (4, B, T, total_pairs)
        out = x_axes[0].clone()
        num_axes = 4
        # axis 1..3
        sec1, sec2, sec3 = int(mrope_section[1]), int(mrope_section[2]), int(mrope_section[3])
        if sec1 > 0:
            out[..., 1 : sec1 * num_axes : num_axes] = x_axes[1, ..., 1 : sec1 * num_axes : num_axes]
        if sec2 > 0:
            out[..., 2 : sec2 * num_axes : num_axes] = x_axes[2, ..., 2 : sec2 * num_axes : num_axes]
        if sec3 > 0:
            out[..., 3 : sec3 * num_axes : num_axes] = x_axes[3, ..., 3 : sec3 * num_axes : num_axes]
        return out
    ...
    def _maybe_init_caches(self, *, device: torch.device, seq_len: int) -> None:
        ...
        if (
            ...
            if self._cos_index.size(-1) != total_pairs:
                raise RuntimeError("Internal error: index cache has wrong last-dim")
        ...
        if (
            ...
            if self._cos_bar.size(-1) != total_pairs:
                raise RuntimeError("Internal error: bar cache has wrong last-dim")
        ...
        if (
            ...
            if self._cos_pos.size(-1) != total_pairs:
                raise RuntimeError("Internal error: pos cache has wrong last-dim")
        ...
        if (
            ...
            if self._cos_pitch.size(-1) != total_pairs:
                raise RuntimeError("Internal error: pitch cache has wrong last-dim")
    ...
    def forward(
        ...
        *,
        ...
        # Clamp to cache size for safety.
        ...
        # --- gather cos/sin per axis (all axes are full head_dim/2) ---
        ...
        # Stack: (4, B, T, total_pairs)
        cos_axes = torch.stack(
            [
                cos_index.squeeze(2),
                cos_bar.squeeze(2),
                cos_pos.squeeze(2),
        ...
        if self.pair_layout == "block":
            cos = self._combine_block(cos_axes, self.mrope_section)
            sin = self._combine_block(sin_axes, self.mrope_section)
        else:
            cos = self._combine_interleave_fixed_stride(cos_axes, self.mrope_section)
            sin = self._combine_interleave_fixed_stride(sin_axes, self.mrope_section)

        # Restore broadcast dim for attention: (B, T, 1, total_pairs)
        cos = cos.unsqueeze(2)
        sin = sin.unsqueeze(2)
        ...
        if cos.size(-1) != (self.head_dim // 2):
            raise RuntimeError(
                f"MRoPE produced wrong last-dim: got {cos.size(-1)}, expected {self.head_dim // 2}. "
...
# -----------------------------------------------------------------------------
# Distributed data loader (BOS-aligned, low overhead)
#
# Special token IDs in this project:
#   BOS     = 161  : sequence/document start token
#   ABS_END = 162  : end token for the ABS (=global_max) header placed at the beginning
#   EOF     = 163  : sequence/document end token (also used as rope_doc_id)
...
def _peek_data_shard(filename: str) -> int:
    # Only reads the header, returns the number of tokens in the shard.
...
def _load_data_shard(file: Path) -> torch.Tensor:
    # Efficient shard loader: read header via torch.from_file, tokens via readinto() into pinned memory.
    header = torch.from_file(str(file), False, 256, dtype=torch.int32)  # header is 256 int32
    assert int(header[0]) == 20240520, "magic number mismatch in the data .bin file"
    assert int(header[1]) == 1, "unsupported version"
...
class BOSFinder:
    ...
    The shard is assumed to contain BOS_ID tokens that mark document/chunk starts.
    next_batch() returns, for every rank, lists of (start, end) spans whose concatenation
    produces exactly (num_tokens_local + 1) tokens.
    """

    def __init__(self, tokens: torch.Tensor, world_size: int = 1, quickload: bool = False):
        self.tokens = tokens
        ...
        if quickload:
            # Scan the first few million tokens immediately, and scan the full shard async.
            scan_n = min(self.size, 4_000_000)
            self.bos_idx = (tokens[:scan_n] == BOS_ID).nonzero(as_tuple=True)[0].to(torch.int64).cpu().numpy()
            self.thread = None
            self.ready = threading.Event()
            self.start()
            # If BOS tokens do not appear early, fall back to full scan to avoid false StopIteration.
    ...
    def next_batch(self, num_tokens_local: int, max_seq_len: int):
        # If quickload was used, repoint to the full dataset after a few batches.
        ...
        for r in range(self.world_size):
            ...
            while cur_len <= num_tokens_local:
                ...
                starts[r].append(cur)
                next_bos = int(self.bos_idx[idx + 1]) if idx + 1 < n else self.size
                end = min(next_bos, cur + max_seq_len, cur + num_tokens_local - cur_len + 1)
                ends[r].append(int(end))
                cur_len += end - cur
                idx += 1
...
class DataPreloader:
    """Helper for asynchronously loading the next shard and indexing BOS tokens."""
...
class DistributedDataLoader:
    """DDP-aware loader with optional BOS alignment.

    - align_to_bos=True (default):
        Streams BOS-delimited documents.

        If `preserve_overflow=True` (default), **no tokens are dropped**:
        when the fixed (T+1) buffer boundary cuts a document, the remainder is
        carried into the next batch.

        Note the unavoidable trade-off:
          * reset() guarantees the first returned token for each rank is BOS_ID.
          * subsequent batches may start mid-document if the previous batch ended mid-document.
            (Eliminating token loss while keeping fixed-length batches requires this.)

        If `preserve_overflow=False`, this falls back to the previous behavior:
        each batch starts at BOS_ID but any truncated tail of the last packed span is discarded.

    - align_to_bos=False:
        Falls back to the original contiguous slicing scheme.

    Public API: reset(), next_batch(), plus ntok_total/files.
    """
    ...
    def __init__(
        ...
        *,
        ...
        quickload: bool = True,
        preserve_overflow: bool = True,
    ):
        self.process_rank = process_rank
        self.num_processes = num_processes
        self.T = int(T)
        self.align_to_bos = bool(align_to_bos)
        self.max_seq_len = int(max_seq_len) if max_seq_len is not None else (self.T + 1)
        self.quickload = bool(quickload)
        self.preserve_overflow = bool(preserve_overflow)

        # Glob files that match the pattern.
        self.files = sorted(glob.glob(filename_pattern))
        assert len(self.files) > 0, f"did not find any files that match the pattern {filename_pattern}"
        self._paths = [Path(f) for f in self.files]

        # Load and validate all data shards, count number of tokens in total.
        ntok_total = 0
        min_shard_ntok = (self.num_processes * (self.T + 1)) if self.align_to_bos else (self.num_processes * self.T + 1)
        for fname in self.files:
            shard_ntok = _peek_data_shard(fname)
            assert shard_ntok >= min_shard_ntok, (
                f"shard too small for the requested configuration: {fname} has {shard_ntok} tokens, "
                f"but need at least {min_shard_ntok}"
            )
            ntok_total += int(shard_ntok)
        self.ntok_total = ntok_total

        # Double-buffered pinned host buffers + device buffers (safe reuse via CUDA events).
        self._cpu_u16 = [torch.empty(self.T + 1, dtype=torch.uint16, pin_memory=True) for _ in range(2)]
        self._cpu_x = [torch.empty(self.T, dtype=torch.int32, pin_memory=True) for _ in range(2)]
        self._cpu_y = [torch.empty(self.T, dtype=torch.int64, pin_memory=True) for _ in range(2)]
        self._gpu_x = [torch.empty(self.T, dtype=torch.int32, device="cuda") for _ in range(2)]
        self._gpu_y = [torch.empty(self.T, dtype=torch.int64, device="cuda") for _ in range(2)]
        self._copy_done = [None, None]
        self._buf_idx = 0

        # BOS-stream state (used only when align_to_bos and preserve_overflow are enabled)
        self._bos_idx = None  # numpy array
        self._bos_n = 0
        self._doc_ptr = 0     # index into bos_idx (document start index)
        self._pos = -1        # current token position within `tokens` (>=0) or -1 means "need init"

        self.reset()

    def reset(self):
        # Rebuild the (cycling) shard iterator so reset() always restarts from the first shard.
        self._file_iter = cycle(self._paths)
        self.tokens = _load_data_shard(next(self._file_iter))

        if self.align_to_bos:
            self.finder = BOSFinder(self.tokens, world_size=self.num_processes, quickload=self.quickload)
            self.preloader = DataPreloader(self._file_iter, self.num_processes)
            self.preloader.start()
            if self.preserve_overflow:
                self._init_bos_stream_state()
        else:
            self.current_position = self.process_rank * self.T

    def _advance_unaligned(self):
        self.tokens = _load_data_shard(next(self._file_iter))
        self.current_position = self.process_rank * self.T

    def _promote_full_bos(self) -> None:
        """If BOS quickload is in progress, wait for the full scan and refresh bos indices."""
        # BOSFinder.get() is a no-op if no async scan is active.
        try:
            self.finder.get()
        except Exception:
            # Defensive: if the finder doesn't support get() for some reason, ignore.
            pass
        self._bos_idx = self.finder.bos_idx
        self._bos_n = len(self._bos_idx)

    def _init_bos_stream_state(self) -> None:
        """Initialize rank-local BOS-stream cursor for the current shard."""
        # If quickload's partial scan doesn't contain enough BOS for all ranks, block until full scan.
        if len(self.finder.bos_idx) < self.num_processes:
            self._promote_full_bos()
        else:
            self._bos_idx = self.finder.bos_idx
            self._bos_n = len(self._bos_idx)

        if self._bos_n < self.num_processes:
            raise RuntimeError(
                f"align_to_bos requires >= world_size={self.num_processes} BOS tokens per shard; got {self._bos_n}. "
                "(Either regenerate shards with more BOS-delimited docs, or set align_to_bos=False / preserve_overflow=False.)"
            )

        self._doc_ptr = int(self.process_rank)
        self._pos = int(self._bos_idx[self._doc_ptr])

    def _advance_bos_shard(self) -> None:
        """Swap in the preloaded shard and reinitialize BOS-stream state."""
        self.tokens, self.finder = self.preloader.get()
        self.preloader.start()
        # Preloaded shards are fully indexed, but keep the refresh logic unified.
        self._promote_full_bos()
        if self._bos_n < self.num_processes:
            raise RuntimeError(
                f"align_to_bos requires >= world_size={self.num_processes} BOS tokens per shard; got {self._bos_n} in shard swap."
            )
        self._doc_ptr = int(self.process_rank)
        self._pos = int(self._bos_idx[self._doc_ptr])

    def next_batch(self):
        # Ensure we don't overwrite pinned host buffers that still have an in-flight H2D copy.
        slot = self._buf_idx
        ev = self._copy_done[slot]
        if ev is not None:
            ev.synchronize()

        buf_u16 = self._cpu_u16[slot]

        if self.align_to_bos:
            if self.preserve_overflow:
                # Build a BOS-aware stream buffer of exactly (T + 1) tokens for this rank.
                dst = 0
                need = self.T + 1
                while dst < need:
                    # Ensure we have a valid document pointer.
                    if self._doc_ptr >= self._bos_n:
                        # If quickload is still running, complete the scan before declaring shard exhaustion.
                        self._promote_full_bos()

                    if self._doc_ptr >= self._bos_n:
                        # Shard exhausted for this rank: move to the next shard.
                        self._advance_bos_shard()
                        continue

                    # If we're at a doc boundary and need to (re)initialize position.
                    if self._pos < 0:
                        self._pos = int(self._bos_idx[self._doc_ptr])

                    # Ensure doc_end is computed from a complete BOS index list if we're at the tail.
                    if (self._doc_ptr + 1) >= self._bos_n:
                        self._promote_full_bos()

                    doc_end = int(self._bos_idx[self._doc_ptr + 1]) if (self._doc_ptr + 1) < self._bos_n else self.tokens.numel()

                    take = min(doc_end - self._pos, need - dst)
                    if take <= 0:
                        # Defensive: if we somehow landed at/past doc_end, jump to next doc.
                        self._doc_ptr += self.num_processes
                        self._pos = -1
                        continue

                    buf_u16[dst:dst + take].copy_(self.tokens[self._pos:self._pos + take])
                    self._pos += take
                    dst += take

                    if self._pos >= doc_end:
                        # Finished current doc: move to next doc assigned to this rank.
                        self._doc_ptr += self.num_processes
                        self._pos = -1

                if dst != need:
                    raise RuntimeError(f"BOS stream batch construction failed: expected {need} tokens, got {dst}")

            else:
                # Previous behavior: each batch starts at BOS_ID, but a truncated tail is discarded.
                while True:
                    try:
                        seq_starts, seq_ends = self.finder.next_batch(self.T, self.max_seq_len)
                        starts = seq_starts[self.process_rank]
                        ends = seq_ends[self.process_rank]
                        break
                    except StopIteration:
                        # Shard exhausted: swap to the preloaded shard and start preloading the next.
                        self.tokens, self.finder = self.preloader.get()
                        self.preloader.start()
                ...
                for s, e in zip(starts, ends):
                    seg_len = e - s
                    buf_u16[dst:dst + seg_len].copy_(self.tokens[s:e])
                    dst += seg_len
                if dst != self.T + 1:
                    raise RuntimeError(
                        f"BOS-aligned batch construction failed: expected {self.T + 1} tokens, got {dst}"
                    )

        else:
            # Original contiguous slicing scheme.
            batch_size = self.T * self.num_processes
            start = self.current_position
            buf_u16.copy_(self.tokens[start:start + self.T + 1])
            self.current_position += batch_size
            if self.current_position + batch_size >= self.tokens.numel():
                self._advance_unaligned()

        # Cast on CPU before transfer (avoids dtype conversion during H2D).
        x_cpu = self._cpu_x[slot]
        y_cpu = self._cpu_y[slot]
        x_cpu.copy_(buf_u16[:-1])
        y_cpu.copy_(buf_u16[1:])

        # Async H2D into persistent device buffers.
        x_gpu = self._gpu_x[slot]
        y_gpu = self._gpu_y[slot]
        x_gpu.copy_(x_cpu, non_blocking=True)
        y_gpu.copy_(y_cpu, non_blocking=True)

        if self._copy_done[slot] is None:
            self._copy_done[slot] = torch.cuda.Event()
        self._copy_done[slot].record()  # marks completion of the copies on the current stream
...
class Hyperparameters:
    ...
    sequence_length : int = 2048 * 64   # sequence length, in tokens
    num_iterations : int = 8000 # number of iterations to run
    warmup_iters : int = 0
    ...
    val_tokens : int = 2048 * 64 # how many tokens of validation data? it's important to keep this fixed for consistent comparisons
