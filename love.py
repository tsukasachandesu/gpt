...
class Yarn(nn.Module):
    ...
    def forward(
        ...
        token_ids: Tensor,
        *,
        return_ivl_prev_idx: bool = False,
        ...
        # For interval_prev absolute positional embedding (APE), we may need ivl_prev_idx
        # even when the rotary axis itself is disabled.
        need_ivl_prev_idx = return_ivl_prev_idx or use_ip
        ...
        if (not need_extra) and (not need_ivl_prev_idx):
        ...
        if use_is or need_ivl_prev_idx:
            ...
            # interval_prev (for rotary axis and/or for absolute positional embedding):
            # diff to previous skyline snapshot at POS (piece start => index 0)
            if need_ivl_prev_idx:
        ...
        if return_ivl_prev_idx:
            # Always return a valid index tensor (0 for "non pitch / no prev").
            if ivl_prev_idx is None:
                ivl_prev_idx = torch.zeros((T,), device=tok.device, dtype=torch.int32)
            return cos4, sin4, ivl_prev_idx
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, num_kv_heads: int, head_dim: int, model_dim: int, max_seq_len: int):
        ...
        # interval_prev absolute positional embedding (APE)
        # index 0 is reserved for "non pitch / no prev" and kept fixed via padding_idx.
        self.ivl_prev_ape = nn.Embedding(self.yarn.ivl_prev_cache_len, model_dim, padding_idx=0)
    ...
    def init_weights(self):
        ...
        torch.nn.init.normal_(self.ivl_prev_ape.weight, mean=0.0, std=1.0)
        if self.ivl_prev_ape.padding_idx is not None:
            self.ivl_prev_ape.weight[self.ivl_prev_ape.padding_idx].zero_()
    ...
    def setup_optimizers(
        ...
        embedding_params = list(self.transformer.wte.parameters()) + list(self.ivl_prev_ape.parameters())
    ...
    def forward(
        ...
        # YaRN: generate cos/sin once per sequence (and interval_prev indices for APE)
        cos, sin, ivl_prev_idx = self.yarn(input_seq, return_ivl_prev_idx=True)

        x = self.transformer.wte(input_seq)
        # interval_prev absolute positional embedding (APE)
        x.add_(self.ivl_prev_ape(ivl_prev_idx))
        x = norm(x)[None]
        x0 = x
...
class Hyperparameters:
    ...
    ws_schedule: tuple = (20,20)
    ws_final: int = 20 # set final validation ws, used for YaRN extension and short window size
...
class YarnState:
    ...
    **重要**: 本実装は Yarn.forward() の定義と厳密に一致することを優先する。
    特に skyline_tok の定義は forward と同じく
        skyline_tok = pitch_tok & prev_is_pos
    （「直前トークンが POS のときだけ」）であり、
    POS の後に別トークンが挟まった pitch を skyline 扱いにはしない。
    ...
    def __init__(self, yarn: "Yarn", *, device: torch.device, dtype: torch.dtype):
        ...
        # Cache common ints (reduce per-token Python overhead).
        self._pitch_start = int(y.pitch_start)
        self._pitch_end = int(y.pitch_start + y.pitch_size)
        self._pos_start = int(y.pos_start)
        self._pos_end = int(y.pos_start + y.pos_size)
        self._bar_id = int(y.bar_id) if y.bar_id is not None else None
        self._doc_id = int(y.doc_id) if y.doc_id is not None else None
        self._pos_idx_offset = int(y.pos_idx_offset)
        self._pitch_idx_offset = int(y.pitch_idx_offset)
        self._pos_cache_max = int(self._cos_pos.size(0) - 1)
        self._pitch_cache_max = int(self._cos_pitch.size(0) - 1)
        self._ivl_same_min = int(y.ivl_same_min)
        self._ivl_prev_min = int(y.ivl_prev_min)
        self._ivl_idx_offset = int(y.ivl_idx_offset)
        self._ivl_same_cache_max = int(self._cos_ivl_same.size(0) - 1)
        self._ivl_prev_cache_max = int(self._cos_ivl_prev.size(0) - 1)

        # Cache axis layout helpers
        self._stride = int(len(y.mrope_section))
        self._s0, self._s1, self._s2, self._s3, self._s4, self._s5 = (int(x) for x in y.mrope_section)

        if y.pair_layout == "block":
            s0, s1, s2, s3, s4, _s5 = self._s0, self._s1, self._s2, self._s3, self._s4, self._s5
            self._o1 = s0
            self._o2 = s0 + s1
            self._o3 = self._o2 + s2
            self._o4 = self._o3 + s3
            self._o5 = self._o4 + s4
    ...
    def reset(self):
        ...
        # interval state
        # last skyline pitch id across the stream (offset-coded: pitch_abs+1; 0 means "none"),
        # reset only on doc (matches forward: update_mask = skyline_tok | doc_tok).
        self.prev_skyline_id = 0

        # snapshot of prev_skyline_id at the last POS (reset on bar/doc)
        self.seg_prev_skyline_id = 0

        # skyline pitch (raw pitch_abs, 0 is both a valid pitch and "none" like forward)
        # forward definition: skyline_pitch = forward_fill_from_updates(..., skyline_tok|pos|bar|doc)
        self.skyline_pitch_abs = 0

        # whether the previous token was a POS token (for skyline_tok detection)
        self.prev_is_pos = False
        self.last_ivl_prev_idx = 0
    ...
    def init_from_prompt(self, token_ids: torch.Tensor):
        ...
        T = int(tok.numel())
        idx = getattr(self.yarn, "_idx_cache", None)
        if isinstance(idx, torch.Tensor) and idx.device == tok.device and idx.numel() >= T and idx.dtype == torch.int32:
            idx = idx[:T]
        else:
            idx = torch.arange(T, device=tok.device, dtype=torch.int32)

        y = self.yarn
        pitch_tok = (tok >= self._pitch_start) & (tok < self._pitch_end)
        pos_tok = (tok >= self._pos_start) & (tok < self._pos_end)
        bar_tok = tok.eq(self._bar_id) if self._bar_id is not None else torch.zeros_like(tok, dtype=torch.bool)
        doc_tok = tok.eq(self._doc_id) if self._doc_id is not None else torch.zeros_like(tok, dtype=torch.bool)

        # pos axis state (already includes offset)
        pos_updates = torch.where(
            pos_tok,
            (tok - self._pos_start + self._pos_idx_offset).to(torch.int32),
            torch.zeros_like(tok, dtype=torch.int32),
        )
        pos_update_mask = pos_tok | bar_tok | doc_tok
        pos_idx = forward_fill_from_updates(pos_updates, pos_update_mask, idx)
        ...
        pitch_updates = torch.where(
            pitch_tok,
            (tok - self._pitch_start + self._pitch_idx_offset).to(torch.int32),
            torch.zeros_like(tok, dtype=torch.int32),
        )
        ...
        prev_is_pos = torch.cat([pos_tok.new_zeros((1,)), pos_tok[:-1]], dim=0)
        skyline_tok = pitch_tok & prev_is_pos
        pitch_abs = (tok - self._pitch_start).to(torch.int32)
        ...
        # per-segment previous skyline id snapshot (updated at POS, reset on bar/doc)
        prev_seg_updates = torch.where(pos_tok, last_skyline_id, torch.zeros_like(pitch_abs))
        prev_seg_id = forward_fill_from_updates(prev_seg_updates, pos_tok | bar_tok | doc_tok, idx)
        self.seg_prev_skyline_id = int(prev_seg_id[-1].item())

        # current segment skyline pitch (raw pitch_abs), reset at pos/bar/doc
        skyline_updates = torch.where(skyline_tok, pitch_abs, torch.zeros_like(pitch_abs))
        skyline_pitch = forward_fill_from_updates(skyline_updates, skyline_tok | pos_tok | bar_tok | doc_tok, idx)
        self.skyline_pitch_abs = int(skyline_pitch[-1].item())

        # prev_is_pos for the *next* token
        self.prev_is_pos = bool(pos_tok[-1].item())
    ...
    def step(self, tok_id: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Side effect:
          self.last_ivl_prev_idx is updated (0 means "non pitch / no prev").
        ...
        # token type flags
        pitch_tok = (tok >= self._pitch_start) and (tok < self._pitch_end)
        pos_tok = (tok >= self._pos_start) and (tok < self._pos_end)
        bar_tok = (tok == self._bar_id) if (self._bar_id is not None) else False
        doc_tok = (tok == self._doc_id) if (self._doc_id is not None) else False

        # skyline token: first pitch immediately following a POS token (matches forward)
        skyline_tok = pitch_tok and bool(self.prev_is_pos)

        # --- update states (pos/pitch/interval) ---

        # POS axis state (forward-fill with resets on bar/doc)
        if doc_tok or bar_tok:
            self.pos_state = 0
        elif pos_tok:
            self.pos_state = tok - self._pos_start + self._pos_idx_offset

        # Pitch axis state (pre-gate)
        if not bool(y.carry_pitch):
            self.pitch_state = (tok - self._pitch_start + self._pitch_idx_offset) if pitch_tok else 0
        else:
            reset = doc_tok
            ...
            if reset:
                self.pitch_state = 0
            if pitch_tok:
                self.pitch_state = tok - self._pitch_start + self._pitch_idx_offset

        # Interval / skyline states:
        # * prev_skyline_id: reset only on doc, update on skyline_tok
        # * seg_prev_skyline_id: update on pos, reset on bar/doc
        # * skyline_pitch_abs: update on skyline_tok, reset on pos/bar/doc
        if doc_tok:
            self.prev_skyline_id = 0
            self.seg_prev_skyline_id = 0
            self.skyline_pitch_abs = 0
            ...
            if skyline_tok:
                pitch_abs = tok - self._pitch_start  # 0..pitch_size-1
                self.prev_skyline_id = pitch_abs + 1
                self.skyline_pitch_abs = pitch_abs
        ...
        if pitch_tok:
            ...
            # interval_same: diff to current skyline (forward uses skyline_pitch_abs which may be 0)
            diff_same = int(pitch_abs) - int(self.skyline_pitch_abs)
            ivl_same_idx = diff_same - int(self._ivl_same_min) + int(self._ivl_idx_offset)
            if ivl_same_idx < 0:
                ivl_same_idx = 0
            elif ivl_same_idx > self._ivl_same_cache_max:
                ivl_same_idx = self._ivl_same_cache_max

            # interval_prev: diff to previous skyline snapshot at POS (piece start => non pitch)
            if int(self.seg_prev_skyline_id) > 0:
                prev_abs = int(self.seg_prev_skyline_id) - 1
                diff_prev = int(pitch_abs) - prev_abs
                ivl_prev_idx = diff_prev - int(self._ivl_prev_min) + int(self._ivl_idx_offset)
                if ivl_prev_idx < 0:
                    ivl_prev_idx = 0
                elif ivl_prev_idx > self._ivl_prev_cache_max:
                    ivl_prev_idx = self._ivl_prev_cache_max
            else:
                ivl_prev_idx = 0
        ...
        # clamp indices (pos/pitch)
        pos_idx = int(self.pos_state)
        if pos_idx < 0:
            pos_idx = 0
        elif pos_idx > self._pos_cache_max:
            pos_idx = self._pos_cache_max

        if pitch_idx < 0:
            pitch_idx = 0
        elif pitch_idx > self._pitch_cache_max:
            pitch_idx = self._pitch_cache_max
        ...
        stride = self._stride
        s0, s1, s2, s3, s4, s5 = self._s0, self._s1, self._s2, self._s3, self._s4, self._s5
        ...
        # advance
        self.idx_pos += 1
        self.prev_is_pos = bool(pos_tok)

        self.last_ivl_prev_idx = int(ivl_prev_idx)
...
def gpt_forward_infer_logits(
    ...
    *,
    ...
    # interval_prev absolute positional embedding (APE)
    if isinstance(ivl_prev_idx, int):
        if T != 1:
            raise ValueError("ivl_prev_idx=int is only valid when T==1")
        x.add_(model.ivl_prev_ape.weight[ivl_prev_idx])
    else:
        x.add_(model.ivl_prev_ape(ivl_prev_idx))
    x = norm(x)[None]
    x0 = x
...
def _run_inference_cli():
    ...
    # Backward-compat: if the checkpoint predates ivl_prev_ape, keep it disabled (all zeros).
    if "ivl_prev_ape.weight" in missing:
        with torch.no_grad():
            model.ivl_prev_ape.weight.zero_()
    ...
    model.ivl_prev_ape.to(dtype=amp_dtype)
    ...
    if use_kv_cache:
        ...
        with torch.no_grad(), autocast_ctx:
            cos, sin, ivl_prev_idx = model.yarn(prompt, return_ivl_prev_idx=True)
        ...
        for _ in range(int(a.max_new_tokens)):
            ...
            ivl_prev_idx1 = yarn_state.last_ivl_prev_idx
        ...
        for _ in range(int(a.max_new_tokens)):
            ...
            with torch.no_grad(), autocast_ctx:
                cos, sin, ivl_prev_idx = model.yarn(seq, return_ivl_prev_idx=True)
                cos = cos.to(dtype=amp_dtype)
                sin = sin.to(dtype=amp_dtype)
                logits = gpt_forward_infer_logits(
                    model,
...
if model.transformer.wte.weight.device.type == "cuda":
    ...
    model.ivl_prev_ape.to(dtype=torch.bfloat16)
    for ve in model.value_embeds.values():
...
def load_checkpoint(*, model: nn.Module, training_manager: TrainingManager, train_loader: ResumableDistributedDataLoader) -> int:
    ...
    if master_process:
        ...
        # Backward-compat: allow resuming from checkpoints that predate ivl_prev_ape
        if isinstance(model_state, dict) and ("ivl_prev_ape.weight" not in model_state) and hasattr(model_for_state, "ivl_prev_ape"):
            # Keep it disabled by default (all zeros) when resuming from older checkpoints.
            model_state["ivl_prev_ape.weight"] = torch.zeros_like(model_for_state.ivl_prev_ape.weight, device="cpu")
