...
class ModelConfig:
    ...
    vocab_size: int  # output vocab size (lm_head / loss / sampling)
    num_layers: int = 16
    num_heads: int = 8
    head_dim: int = 128
    model_dim: int = 1024
...
REMI96 = Remi96Spec()
BASE_VOCAB_SIZE = int(REMI96.vocab_size)  # absolute REMI tokens (0..131)
PITCH_DELTA_MIN = -(int(REMI96.pitch_size) - 1)  # -95
PITCH_DELTA_MAX = int(REMI96.pitch_size) - 1      # +95
PITCH_DELTA_SIZE = int(PITCH_DELTA_MAX - PITCH_DELTA_MIN + 1)  # 191
INPUT_VOCAB_SIZE = int(BASE_VOCAB_SIZE)  # 132 (input tokens are absolute REMI only)
OUTPUT_NON_PITCH_OFFSET = int(PITCH_DELTA_SIZE)  # 191
OUTPUT_NON_PITCH_SIZE = int(BASE_VOCAB_SIZE - int(REMI96.bar_id))  # BAR..PAD => 36
OUTPUT_VOCAB_SIZE = int(OUTPUT_NON_PITCH_OFFSET + OUTPUT_NON_PITCH_SIZE)  # 227
...
def _output_non_pitch_from_input_id(tok_in: int, *, remi: Remi96Spec = REMI96) -> int:
    ti = int(tok_in)
    lo = int(remi.bar_id)
    hi = int(BASE_VOCAB_SIZE)
    if ti < lo or ti >= hi:
        raise ValueError(f"non-pitch input token id out of range: {ti} (expected {lo}..{hi-1})")
    return int(OUTPUT_NON_PITCH_OFFSET + (ti - lo))
...
def _forward_fill_with_default(
    updates: Tensor,
    update_mask: Tensor,
    idx: Tensor,
    *,
    ...
    """Forward-fill updates and use `default` before the first update."""
    upd_idx = torch.where(update_mask, idx, idx.new_full((), -1).expand_as(idx))
    last_idx = torch.cummax(upd_idx, dim=0)[0]
    last_idx_clamped = last_idx.clamp_min(0).to(dtype=torch.int64)
    out = updates.gather(0, last_idx_clamped)
    return torch.where(last_idx >= 0, out, torch.full_like(out, int(default)))


def build_pitch_delta_targets(input_seq: Tensor, target_seq: Tensor) -> Tensor:
    """Build output-vocab targets (delta-only pitch + compact non-pitch ids).

    Rule:
      * pitch targets: always converted to delta id
          - if previous pitch/skyline exists: delta is relative to that pitch
          - otherwise (document-head first pitch): use base_abs=0
      * non-pitch targets (BAR/POS/BOS/EOF/PAD): remapped to compact output ids
    """
    assert input_seq.ndim == 1 and target_seq.ndim == 1 and input_seq.numel() == target_seq.numel()
    T = int(input_seq.numel())
    if T == 0:
        return target_seq.new_zeros((0,), dtype=torch.int64)

    tok_in = input_seq.to(torch.int32) if input_seq.dtype != torch.int32 else input_seq
    tok_tgt = target_seq.to(torch.int32) if target_seq.dtype != torch.int32 else target_seq
    idx = torch.arange(T, device=tok_in.device, dtype=torch.int32)

    pitch_tok_in = (tok_in >= int(REMI96.pitch_min)) & (tok_in <= int(REMI96.pitch_max))
    pos_tok_in = (tok_in >= int(REMI96.pos_offset)) & (tok_in < int(REMI96.pos_offset + REMI96.pos_size))
    bos_tok_in = tok_in == int(REMI96.bos_id)
    prev_is_pos_in = torch.cat(
        (torch.zeros((1,), dtype=torch.bool, device=tok_in.device), pos_tok_in[:-1]),
        dim=0,
    )
    skyline_tok_in = pitch_tok_in & prev_is_pos_in

    pitch_abs_in = (tok_in - int(REMI96.pitch_min)).to(torch.int32)
    prev_pitch_updates = torch.where(pitch_tok_in, pitch_abs_in, torch.full_like(tok_in, -1))
    prev_pitch_mask = pitch_tok_in | bos_tok_in
    prev_pitch_abs = _forward_fill_with_default(prev_pitch_updates, prev_pitch_mask, idx, default=-1).to(torch.int32)

    prev_sky_updates = torch.where(skyline_tok_in, pitch_abs_in, torch.full_like(tok_in, -1))
    prev_sky_mask = skyline_tok_in | bos_tok_in
    prev_skyline_abs = _forward_fill_with_default(prev_sky_updates, prev_sky_mask, idx, default=-1).to(torch.int32)

    pitch_tok_tgt = (tok_tgt >= int(REMI96.pitch_min)) & (tok_tgt <= int(REMI96.pitch_max))
    skyline_tok_tgt = pitch_tok_tgt & pos_tok_in

    target_abs = (tok_tgt - int(REMI96.pitch_min)).to(torch.int32)
    base_abs = torch.where(skyline_tok_tgt, prev_skyline_abs, prev_pitch_abs)
    # For document-head first pitch (or malformed prompt without previous pitch), use base_abs=0.
    base_abs = torch.where(base_abs >= 0, base_abs, torch.zeros_like(base_abs))
    delta = (target_abs - base_abs).to(torch.int32)
    delta_out_ids = (delta - int(PITCH_DELTA_MIN)).to(torch.int64)  # 0..190

    non_pitch_tok_tgt = (tok_tgt >= int(REMI96.bar_id)) & (tok_tgt < int(BASE_VOCAB_SIZE))
    non_pitch_out_ids = (int(OUTPUT_NON_PITCH_OFFSET) + (tok_tgt - int(REMI96.bar_id))).to(torch.int64)

    out = torch.zeros((T,), dtype=torch.int64, device=tok_tgt.device)
    out = torch.where(non_pitch_tok_tgt, non_pitch_out_ids, out)
    out = torch.where(pitch_tok_tgt, delta_out_ids, out)
    return out
...
class GPT(nn.Module):
    def __init__(
        ...
        self.vocab_size = int(output_vocab_size)
        self.padded_input_vocab_size = next_multiple_of_n(self.input_vocab_size, n=64)
        self.padded_vocab_size = next_multiple_of_n(self.vocab_size, n=64)

        self.transformer = nn.ModuleDict({
            "wte": nn.Embedding(self.padded_input_vocab_size, model_dim),
        ...
        self.value_embeds = nn.ModuleDict({
            str(i): nn.Embedding(self.padded_input_vocab_size, kv_dim)
    ...
    def forward(
        ...
        softcap = 15.0
        target_for_loss = build_pitch_delta_targets(input_seq, target_seq)

        if not self.training:
            x_flat = x.flatten(end_dim=1)
            x_chunks = x_flat.chunk(4)
            t_chunks = target_for_loss.chunk(4)
            total = target_for_loss.numel()
            ...
            for x_chunk, t_chunk in zip(x_chunks, t_chunks):
                ...
                ce_sum += F.cross_entropy(logits, t_chunk, reduction="sum")
        ...
        ce_loss = F.cross_entropy(logits_for_loss, target_for_loss, reduction="mean")
        return ce_loss
# -----------------------------------------------------------------------------
...
def _delta_pitch_states_from_prompt(tokens: Sequence[int], *, remi: Remi96Spec) -> tuple[int | None, int | None]:
    """Return (latest absolute pitch, latest skyline pitch) after the last BOS in prompt."""
    last_pitch_abs = None
    last_skyline_abs = None
    prev_is_pos = False
    for tok in tokens:
        ti = int(tok)
        if ti == int(remi.bos_id):
            last_pitch_abs = None
            last_skyline_abs = None
            prev_is_pos = False
            continue
        if int(remi.pitch_min) <= ti <= int(remi.pitch_max):
            last_pitch_abs = ti - int(remi.pitch_min)
            if prev_is_pos:
                last_skyline_abs = last_pitch_abs
            prev_is_pos = False
            continue
        prev_is_pos = int(remi.pos_offset) <= ti < int(remi.pos_offset + remi.pos_size)
    return last_pitch_abs, last_skyline_abs
...
def _build_remi_constraint_masks(
    *,
    ...
    out_non_pitch_off = int(OUTPUT_NON_PITCH_OFFSET)
    pos_out_start = int(out_non_pitch_off + (pos_offset - bar_id))
    bar_out_id = int(out_non_pitch_off)
    eof_out_id = _output_non_pitch_from_input_id(int(eof_id), remi=remi)

    mask_pos_all = torch.zeros((vocab_size,), device=device, dtype=torch.bool)
    mask_pos_all[pos_out_start : pos_out_start + pos_size] = True
    mask_bar = torch.zeros((vocab_size,), device=device, dtype=torch.bool)
    mask_bar[bar_out_id] = True
    mask_eof = torch.zeros((vocab_size,), device=device, dtype=torch.bool)
    mask_eof[eof_out_id] = True
    ...
    mask_pos_gt[:, pos_out_start : pos_out_start + pos_size] = pos_gt

    pitch_ids = torch.arange(pitch_size, device=device, dtype=torch.int64)
    last_pitch_vals = torch.arange(pitch_size + 1, device=device)
    pitch_lt = pitch_ids[None, :] < last_pitch_vals[:, None]

    # Delta-space pitch masks indexed by previous absolute pitch (0..95).
    pitch_abs_ids = pitch_ids
    prev_abs_ids = pitch_ids
    delta = pitch_abs_ids[None, :] - prev_abs_ids[:, None]  # [prev_abs, pitch_abs]
    delta_out_ids = (delta - int(PITCH_DELTA_MIN)).to(torch.int64)  # [prev_abs, pitch_abs] in 0..190

    mask_delta_all = torch.zeros((pitch_size, vocab_size), device=device, dtype=torch.bool)
    mask_delta_all.scatter_(1, delta_out_ids, True)

    # AFTER_PITCH constraint in delta space: next pitch must be < last_pitch.
    mask_delta_lt = torch.zeros((pitch_size, pitch_size + 1, vocab_size), device=device, dtype=torch.bool)
    delta_ids_exp = delta_out_ids[:, None, :].expand(pitch_size, pitch_size + 1, pitch_size)
    lt_exp = pitch_lt[None, :, :].expand(pitch_size, pitch_size + 1, pitch_size)
    mask_delta_lt.scatter_(2, delta_ids_exp, lt_exp)

    # No previous pitch/skyline: use base_abs=0, so delta == pitch_abs in [0..95].
    delta_init_ids = (pitch_abs_ids - int(PITCH_DELTA_MIN)).to(torch.int64)
    mask_delta_init = torch.zeros((vocab_size,), device=device, dtype=torch.bool)
    mask_delta_init[delta_init_ids] = True

    mask_delta_init_lt = torch.zeros((pitch_size + 1, vocab_size), device=device, dtype=torch.bool)
    mask_delta_init_lt[:, delta_init_ids] = pitch_lt
    ...
    return dict(
        ...
        eof=mask_eof,
        pos_gt=mask_pos_gt,
        delta_init=mask_delta_init,
        delta_init_lt=mask_delta_init_lt,
        delta_all=mask_delta_all,
        delta_lt=mask_delta_lt,
        pos_size=pos_size,
...
def _remi_state_mask(
    ...
    last_pos: int,
    last_pitch: int | None,
    prev_pitch_abs_for_delta: int | None,
    prev_skyline_abs_for_delta: int | None,
    masks: dict,
) -> torch.Tensor:
    ...
    if state == _REMI_STATE_AFTER_POS:
        if prev_skyline_abs_for_delta is None:
            return masks["delta_init"]
        prev_idx = int(prev_skyline_abs_for_delta)
        if prev_idx < 0 or prev_idx >= pitch_size:
            return masks["delta_init"]
        return masks["delta_all"][prev_idx]
    ...
    if prev_pitch_abs_for_delta is None:
        pitch_mask = masks["delta_init_lt"][idx]
    else:
        prev_idx = int(prev_pitch_abs_for_delta)
        if prev_idx < 0 or prev_idx >= pitch_size:
            pitch_mask = masks["delta_init_lt"][idx]
        else:
            pitch_mask = masks["delta_lt"][prev_idx, idx]
...
def _decode_sampled_output_to_input_token(
    sampled_id: int,
    *,
    remi_state: int,
    prev_pitch_abs_for_delta: int | None,
    ...
    """Map sampled output-vocab id to the input-token id space."""
    sid = int(sampled_id)
    if 0 <= sid < int(PITCH_DELTA_SIZE):
        base_abs = prev_skyline_abs_for_delta if remi_state == _REMI_STATE_AFTER_POS else prev_pitch_abs_for_delta
        if base_abs is None:
            # Initial pitch at document head: delta is interpreted from base_abs=0.
            base_abs = 0
        delta = sid + int(PITCH_DELTA_MIN)
        pitch_abs = int(base_abs) + int(delta)
        pitch_abs = max(0, min(int(remi.pitch_size) - 1, pitch_abs))
        return int(remi.pitch_min) + int(pitch_abs)
    if int(OUTPUT_NON_PITCH_OFFSET) <= sid < int(OUTPUT_VOCAB_SIZE):
        return int(remi.bar_id) + (sid - int(OUTPUT_NON_PITCH_OFFSET))
    raise ValueError(
        f"sampled output token id out of range: {sid} "
        f"(expected 0..{int(OUTPUT_VOCAB_SIZE)-1})"
    )
...
def _model_to_remi96_ids(tokens_model: Sequence[int], *, shift: int) -> list[int]:
    ...
    本リポジトリの base REMI マッピングでは shift=0 が正しく、この関数は恒等写像になる。
    """
    out = []
    shift = int(shift)
...
def _run_inference_cli():
    ...
    parser.add_argument("--remi_shift", type=int, default=0, help="REMI96->decoded/input token space shift for BAR/POS/BOS/EOF/PAD. この語彙設計では 0 を指定してください。")
    ...
    parser.add_argument("--eof_id", type=int, default=REMI96.eof_id, help="EOF token id in decoded/input token space (default 130).")
    ...
    # Base token mapping sanity (REMI absolute tokens in decoded/input token space).
    if int(a.remi_shift) != 0:
        raise RuntimeError("この語彙設計では --remi_shift は 0 を指定してください。")
    if int(a.eof_id) != int(REMI96.eof_id):
        ...
        raise RuntimeError("この語彙設計では decoded/input 側 EOF は 130 です。--eof_id 130 を指定してください。")
    ...
    expected_padded_vocab = int(next_multiple_of_n(int(MODEL_CFG.vocab_size), n=64))
    expected_padded_input_vocab = int(next_multiple_of_n(int(MODEL_CFG.input_vocab_size), n=64))
    lm_head_w = state_dict.get("lm_head.weight", None) if isinstance(state_dict, dict) else None
    if torch.is_tensor(lm_head_w) and int(lm_head_w.shape[0]) != expected_padded_vocab:
        raise RuntimeError(
            f"Incompatible checkpoint vocab: lm_head.out={int(lm_head_w.shape[0])}, "
            f"expected padded_vocab={expected_padded_vocab} for vocab_size={int(MODEL_CFG.vocab_size)} "
            "(delta-only-output checkpoints required)."
        )
    wte_w = state_dict.get("transformer.wte.weight", None) if isinstance(state_dict, dict) else None
    if torch.is_tensor(wte_w) and int(wte_w.shape[0]) != expected_padded_input_vocab:
        raise RuntimeError(
            f"Incompatible checkpoint vocab: wte.rows={int(wte_w.shape[0])}, "
            f"expected padded_input_vocab={expected_padded_input_vocab} for input_vocab_size={int(MODEL_CFG.input_vocab_size)}."
        )

    state_dict = _strip_state_dict_prefixes(state_dict)

    # Model hyperparams (must match training)
    input_vocab_size = int(MODEL_CFG.input_vocab_size)
    ...
    # Build allowed mask (output vocab):
    #   0..190   : PITCH_DELTA(-95..+95)
    #   191..226 : BAR/POS/BOS/EOF/PAD (mapped from input BAR..PAD)
    ...
    if int(a.disallow_bos_pad) == 1:
        ...
        allowed[_output_non_pitch_from_input_id(BOS_ID)] = False
        allowed[_output_non_pitch_from_input_id(PAD_ID)] = False
    ...
    delta_prev_pitch_abs, delta_prev_skyline_abs = _delta_pitch_states_from_prompt(tokens_model, remi=REMI96)
    if enforce_remi:
    ...
    if use_kv_cache:
        ...
        tok1_buf = torch.empty((1,), device=device, dtype=torch.long)

        # Decode loop
        for _ in range(int(a.max_new_tokens)):
            step_allowed = allowed_mask_base
            if enforce_remi:
                state_mask = _remi_state_mask(
                    remi_state,
                    remi_last_pos,
                    remi_last_pitch,
                    delta_prev_pitch_abs,
                    delta_prev_skyline_abs,
                    remi_masks,
                )
            ...
            sampled_id = int(next_id_t.item())
            next_id = _decode_sampled_output_to_input_token(
                sampled_id,
                remi_state=remi_state,
                prev_pitch_abs_for_delta=delta_prev_pitch_abs,
                prev_skyline_abs_for_delta=delta_prev_skyline_abs,
                remi=REMI96,
            )
            generated.append(next_id)
            remi_state_before = remi_state
            ...
            if int(REMI96.pitch_min) <= next_id <= int(REMI96.pitch_max):
                delta_prev_pitch_abs = int(next_id - int(REMI96.pitch_min))
                if remi_state_before == _REMI_STATE_AFTER_POS:
                    delta_prev_skyline_abs = int(next_id - int(REMI96.pitch_min))
            elif next_id == int(REMI96.bos_id):
                delta_prev_pitch_abs = None
                delta_prev_skyline_abs = None

            cos1, sin1 = yarn_state.step(next_id)
            tok1_buf[0] = int(next_id)
            with torch.no_grad(), autocast_ctx:
...
model: nn.Module = GPT(
    input_vocab_size=int(MODEL_CFG.input_vocab_size),
    output_vocab_size=int(MODEL_CFG.vocab_size),
    num_layers=num_layers,
    num_heads=num_heads,
