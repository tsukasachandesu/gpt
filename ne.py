...
class Yarn(nn.Module):
    ...
    def forward(self, token_ids: Tensor, *, return_pos_ids: bool = False):
        ...
        if not need_extra:
            ...
            cos_out = cos[None, :, None, :]
            sin_out = sin[None, :, None, :]
            if return_pos_ids:
                pos_ids = _compute_pos_ids_for_bias(token_ids, self)
                return cos_out, sin_out, pos_ids
        ...
        if use_pos:
            ...
            if return_pos_ids:
                pos_ids_for_bias = (pos_idx - int(self.pos_idx_offset)).clamp(0, int(self.pos_size) - 1)
                pos_ids_for_bias = pos_ids_for_bias.to(torch.int32)
        ...
        sin_out = sin[None, :, None, :]
        if return_pos_ids:
            if pos_ids_for_bias is None:
                pos_ids_for_bias = _compute_pos_ids_for_bias(token_ids, self)
            return cos_out, sin_out, pos_ids_for_bias
        return cos_out, sin_out
...
class AttnArgs:
    ...
    sin: torch.Tensor
    attn_scale: float
    pos_q_ids: torch.Tensor | None = None
    pos_k_ids: torch.Tensor | None = None
    pos_rel_bias: "PosDeltaRelativeBias | None" = None


def _compute_pos_ids_for_bias(token_ids: torch.Tensor, yarn: "Yarn") -> torch.Tensor:
    """Build per-token POS ids (0..pos_size-1) with forward-fill and bar/doc resets."""
    if token_ids.ndim == 2:
        if token_ids.size(0) != 1:
            raise ValueError(f"token_ids must be (T,) or (1,T), got shape={tuple(token_ids.shape)}")
        token_ids = token_ids.squeeze(0)
    if token_ids.ndim != 1:
    ...
    T = int(tok.numel())
    if T == 0:
        return torch.empty((0,), device=tok.device, dtype=torch.int32)

    idx = torch.arange(T, device=tok.device, dtype=torch.int32)
    pos_start = int(yarn.pos_start)
    pos_size = int(yarn.pos_size)
    pos_tok = (tok >= pos_start) & (tok < (pos_start + pos_size))
    bar_tok = tok.eq(int(yarn.bar_id)) if yarn.bar_id is not None else torch.zeros_like(tok, dtype=torch.bool)
    doc_tok = tok.eq(int(yarn.doc_id)) if yarn.doc_id is not None else torch.zeros_like(tok, dtype=torch.bool)

    pos_updates = torch.where(
        pos_tok,
        (tok - pos_start).to(torch.int32),
        torch.zeros_like(tok, dtype=torch.int32),
    )
    pos_update_mask = pos_tok | bar_tok | doc_tok
    pos_ids = forward_fill_from_updates(pos_updates, pos_update_mask, idx)
    if pos_size > 0:
        pos_ids = pos_ids.clamp(0, pos_size - 1)
    return pos_ids.to(torch.int32)


class PosDeltaRelativeBias(nn.Module):
    """Relative position bias using delta_pos = q_pos - k_pos over bounded integer bins."""

    def __init__(self, *, num_heads: int, min_delta: int, max_delta: int, init_std: float):
        super().__init__()
        self.num_heads = int(num_heads)
        self.min_delta = int(min_delta)
        self.max_delta = int(max_delta)
        if self.max_delta < self.min_delta:
            raise ValueError(f"max_delta ({self.max_delta}) must be >= min_delta ({self.min_delta})")
        self.num_bins = int(self.max_delta - self.min_delta + 1)
        self.bias = nn.Parameter(torch.empty(self.num_heads, self.num_bins))
        self.reset_parameters(float(init_std))

    @torch.no_grad()
    def reset_parameters(self, init_std: float = 0.02) -> None:
        torch.nn.init.normal_(self.bias, mean=0.0, std=float(init_std))

    def build_score_mod(self, q_pos_ids: torch.Tensor, k_pos_ids: torch.Tensor):
        # Captures tensors only; no Python-side per-element branching.
        bias_table = self.bias
        min_delta = int(self.min_delta)
        max_delta = int(self.max_delta)

        def score_mod(score, b, h, q_idx, kv_idx):
            q_pos = q_pos_ids[q_idx].to(torch.int32)
            k_pos = k_pos_ids[kv_idx].to(torch.int32)
            delta = q_pos - k_pos
            delta = torch.clamp(delta, min=min_delta, max=max_delta)
            bin_idx = (delta - min_delta).to(torch.int64)
            rel_bias = bias_table[h, bin_idx].to(dtype=score.dtype)
            return score + rel_bias

        return score_mod


def _dense_to_ordered_kv(dense_blockmask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    num_blocks = dense_blockmask.sum(dim=-1, dtype=torch.int32)
    indices = dense_blockmask.argsort(dim=-1, descending=False, stable=True).flip(-1).to(torch.int32)
    return num_blocks[None, None].contiguous(), indices[None, None].contiguous()


def _validate_cu_seqlens(seqlens: torch.Tensor, total_len: int, *, device: torch.device) -> torch.Tensor:
    total_len = int(total_len)
    if total_len <= 0:
        raise ValueError(f"total_len must be positive, got {total_len}")

    if seqlens.ndim != 1:
        raise ValueError(f"cu_seqlens must be 1-D, got shape={tuple(seqlens.shape)}")

    seqlens_i64 = seqlens.to(device=device, dtype=torch.int64)
    if seqlens_i64.numel() < 2:
        raise ValueError("cu_seqlens must contain at least [0, total_len]")

    deltas = seqlens_i64[1:] - seqlens_i64[:-1]
    is_valid = (
        (seqlens_i64[0] == 0)
        & (seqlens_i64[-1] == total_len)
        & torch.all(deltas > 0)
    )
    if not bool(is_valid):
        raise ValueError(
            "invalid cu_seqlens: expected 1-D strictly increasing sequence starting at 0 "
            f"and ending at total_len={total_len}, got first={int(seqlens_i64[0])}, last={int(seqlens_i64[-1])}"
        )

    return seqlens_i64


def _seqlens_to_doc_ids(
    ...
    *,
    ...
    if validate:
        seqlens_i64 = _validate_cu_seqlens(seqlens, total_len, device=device)
    else:
        seqlens_i64 = seqlens.to(device=device, dtype=torch.int64)
        if seqlens_i64.numel() < 2:
            return torch.zeros((int(total_len),), device=device, dtype=torch.int32)
    # cu_seqlens [0, e1, e2, ...] -> token-wise doc ids [0..num_docs-1]
    doc_ends = seqlens_i64[1:]
    pos = torch.arange(int(total_len), device=device, dtype=torch.int64)
    doc_ids = torch.bucketize(pos, doc_ends, right=True)
    return doc_ids.to(dtype=torch.int32)
...
def _build_causal_window_mask(
    *,
    ...
    def causal_window(b, h, q_idx, kv_idx):
        ...
        kv_abs = kv_idx + kv_offset
        return (q_abs >= kv_abs) & ((q_abs - kv_abs) < window_tokens)
...
def _build_document_block_mask(
    *,
    doc_ids: torch.Tensor,
    window_tokens: int,
    block_size: int,
) -> BlockMask:
    total_len = int(doc_ids.numel())
    if total_len <= 0:
    ...
    def document_causal(b, h, q_idx, kv_idx):
        return (q_idx >= kv_idx) & ((q_idx - kv_idx) < window_tokens) & (doc_ids[q_idx] == doc_ids[kv_idx])

    # Fallback path for non-block-aligned lengths.
    if total_len % block_size != 0:
        dev_str = doc_ids.device.type if isinstance(doc_ids.device, torch.device) else str(doc_ids.device)
        return create_block_mask(
            document_causal,
            B=None,
            H=None,
    ...
    block_idx = torch.arange(num_blocks, dtype=torch.int32, device=doc_ids.device)
    causal_any = block_idx[:, None] >= block_idx
    causal_all = block_idx[:, None] > block_idx

    docs_low = doc_ids.view(-1, block_size)[:, 0].contiguous()
    docs_high = doc_ids.view(-1, block_size)[:, -1].contiguous()
    document_any = (docs_low[:, None] <= docs_high) & (docs_high[:, None] >= docs_low)
    document_all = (docs_low[:, None] == docs_high) & (docs_high[:, None] == docs_low)

    blockmask_any = causal_any & document_any
    blockmask_all = causal_all & document_all

    partial_kv_num_blocks, partial_kv_indices = _dense_to_ordered_kv(blockmask_any & ~blockmask_all)
    full_kv_num_blocks, full_kv_indices = _dense_to_ordered_kv(blockmask_all)

    window_blocks = max(1, int(math.ceil(window_tokens / block_size)))
    window_blocks_t = torch.tensor(window_blocks, dtype=torch.int32, device=doc_ids.device)
    partial_clamped = torch.clamp_max(partial_kv_num_blocks, torch.clamp_min(window_blocks_t - full_kv_num_blocks, 1))
    full_clamped = torch.clamp_max(full_kv_num_blocks, window_blocks_t - 1)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor, attn_args: AttnArgs):
        ...
        score_mod = None
        if (
            attn_args.pos_rel_bias is not None
            and attn_args.pos_q_ids is not None
            and attn_args.pos_k_ids is not None
        ):
            q_pos = attn_args.pos_q_ids[:T]
            k_pos = attn_args.pos_k_ids[:T]
            score_mod = attn_args.pos_rel_bias.build_score_mod(q_pos, k_pos)
        ...
        y = flex_attention(
            ...
            score_mod=score_mod,
            scale=float(attn_args.attn_scale),
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, num_kv_heads: int, head_dim: int, model_dim: int, max_seq_len: int):
        ...
        # POS relative bias (delta_pos = q_pos - k_pos), optionally shared across layers.
        self.pos_rpb: PosDeltaRelativeBias | None = None
        self.pos_rpb_layers: nn.ModuleList | None = None
        if bool(args.pos_rpb_enabled):
            if bool(args.pos_rpb_share_across_layers):
                self.pos_rpb = PosDeltaRelativeBias(
                    num_heads=self.num_heads,
                    min_delta=int(args.pos_rpb_min_delta),
                    max_delta=int(args.pos_rpb_max_delta),
                    init_std=float(args.pos_rpb_init_std),
                )
            else:
                self.pos_rpb_layers = nn.ModuleList([
                    PosDeltaRelativeBias(
                        num_heads=self.num_heads,
                        min_delta=int(args.pos_rpb_min_delta),
                        max_delta=int(args.pos_rpb_max_delta),
                        init_std=float(args.pos_rpb_init_std),
    ...
    def setup_optimizers(
        ...
        pos_rpb_params = list(self.pos_rpb.parameters()) if self.pos_rpb is not None else []
        if self.pos_rpb_layers is not None:
            pos_rpb_params.extend(list(self.pos_rpb_layers.parameters()))
        assert len(list(self.parameters())) == (
            len(matrix_params) + len(embedding_params) + len(lm_head_params) +
            len(value_embeds_params) + len(resid_params) + len(x0_params) + len(pos_rpb_params)
        )
        ...
        if pos_rpb_params:
            adam_groups.append(dict(params=pos_rpb_params, lr=embedding_lr * dmodel_lr_scale))
    ...
    def _build_train_block_masks(self, seqlens: Tensor, bm_sizes: list[int], total_len: int, device: torch.device) -> list[BlockMask]:
        doc_ids = _seqlens_to_doc_ids(seqlens, int(total_len), device=device)
        block_size = int(args.block_size)

        # Build masks once per unique window size; many layers share the same window.
        unique_windows = sorted({int(w) for w in bm_sizes})
        by_window: dict[int, BlockMask] = {}
        for w in unique_windows:
            by_window[w] = _build_document_block_mask(doc_ids=doc_ids, window_tokens=w, block_size=block_size)

        return [by_window[int(w)] for w in bm_sizes]

    def _get_infer_block_masks(self, seq_len: int, ws_short: int, ws_long: int, device: torch.device) -> list[BlockMask]:
        cache = getattr(self, "_infer_bm_cache", None)
        if cache is None:
            cache = OrderedDict()
            setattr(self, "_infer_bm_cache", cache)
        key = (
            int(seq_len),
            int(ws_short),
            int(ws_long),
        ...
        bm_sizes = self._get_bm_sizes(int(ws_short), int(ws_long))
        block_size = int(args.block_size)
        unique_windows = sorted({int(w) for w in bm_sizes})
        by_window: dict[int, BlockMask] = {}
        for w in unique_windows:
            by_window[w] = _build_causal_window_mask(
                q_len=int(seq_len),
                kv_len=int(seq_len),
                window_tokens=w,
                device=device,
        ...
        masks = [by_window[int(w)] for w in bm_sizes]
    ...
    def _get_pos_rpb_for_layer(self, layer_idx: int) -> PosDeltaRelativeBias | None:
        if self.pos_rpb_layers is not None:
            return self.pos_rpb_layers[int(layer_idx)]
        return self.pos_rpb
    ...
    def forward(
        ...
        block_masks = self._build_train_block_masks(seqlens, bm_sizes, int(T), input_seq.device)

        # YaRN + 4-axis MRoPE: generate cos/sin once per sequence and share across layers
        cos, sin, pos_ids = self.yarn(input_seq, return_pos_ids=True)
        ...
        for i, block in enumerate(self.transformer.h):
            ...
            attn_args = AttnArgs(
                ...
                pos_q_ids=pos_ids,
                pos_k_ids=pos_ids,
                pos_rel_bias=self._get_pos_rpb_for_layer(i),
            )
...
def distributed_data_generator(filename_pattern: str, num_tokens: int, max_seq_len: int, grad_accum_steps: int = 1, align_to_bos: bool = True):
    ...
    while True:
        ...
        _validate_cu_seqlens(_cum_lengths, int(num_tokens_local), device=_cum_lengths.device)
...
class Hyperparameters:
    ...
    # POS-relative bias (delta_pos = q_pos - k_pos)
    pos_rpb_enabled: bool = True
    pos_rpb_min_delta: int = -31
    pos_rpb_max_delta: int = 31
    pos_rpb_share_across_layers: bool = True
    pos_rpb_init_std: float = 0.02
...
# -----------------------------------------------------------------------------
# Inference utilities (FlexAttention KV-cache + MIDI)
# -----------------------------------------------------------------------------
from typing import Optional, Sequence, Tuple

class KVCache:
    """Per-layer KV cache for FlexAttention inference.

    Shapes:
      k_cache, v_cache: [num_layers, B, max_seq_len, n_kv_heads, head_dim]
      pos_cache:        [B, max_seq_len] (int32), cached POS ids for relative bias.
    ...
    def allocate(
        *,
        ...
        pos = torch.zeros((batch_size, max_seq_len), device=device, dtype=torch.int32)
        # Keep padding finite to avoid numerical contamination from uninitialized cache tails.
        # Initialize once at allocation time to avoid per-step overhead.
        k.zero_()
        v.zero_()
        cache_seqlens = torch.zeros((batch_size,), device=device, dtype=torch.int32)
        return KVCache(k, v, pos, cache_seqlens)
...
class YarnState:
    ...
    def __init__(self, yarn: "Yarn", *, device: torch.device, dtype: torch.dtype):
        ...
        self._pos_step = torch.empty((1,), device=device, dtype=torch.int32)
    ...
    @torch.no_grad()
    def step(self, tok_id: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Consume one token id and return (cos, sin) for that token position.

        Returns:
          cos, sin: [1, 1, 1, head_dim//2] in self.dtype on device.
          pos_id:   [1] int32 tensor in [0, pos_size-1] for POS-relative bias.
        """
        y = self.yarn
        t = int(self.idx_pos)
        tok = int(tok_id)
        ...
        pos_out = int(self.pos_state) - int(y.pos_idx_offset)
        if pos_out < 0:
            pos_out = 0
        if pos_out >= int(y.pos_size):
            pos_out = int(y.pos_size) - 1
        self._pos_step[0] = int(pos_out)

        return self._cos_step, self._sin_step, self._pos_step


# ---- Inference forward path (FlexAttention + KV cache) ----
...
def _attn_forward_infer(
    ...
    *,
    ...
    pos_cache: torch.Tensor,
    cache_seqlen: int,
    precomputed_block_mask: Optional[BlockMask],
    pos_rel_bias: PosDeltaRelativeBias | None,
) -> torch.Tensor:
    B, T, _ = x.size()
    ...
    enable_gqa = (attn.num_heads != attn.n_kv_head)
    block_size = int(args.block_size)
    window_tokens = max(1, int(window_tokens))

    cache_seqlen = int(cache_seqlen)
    if cache_seqlen + T > int(k_cache.size(1)):
        raise RuntimeError(
            f"KV cache overflow: cache_seqlen={cache_seqlen}, T={T}, max_seq_len={int(k_cache.size(1))}"
        )

    # Append latest K/V into cache.
    k_cache[:, cache_seqlen:cache_seqlen + T].copy_(k)
    v_cache[:, cache_seqlen:cache_seqlen + T].copy_(v)
    pos_cache[:, cache_seqlen:cache_seqlen + T].copy_(pos_ids.to(device=x.device, dtype=torch.int32).view(1, T))

    kv_total = cache_seqlen + T
    # For multi-token queries, keep full KV and let block_mask apply exact per-query windowing.
    kv_start = 0 if T > 1 else max(0, kv_total - window_tokens)
    k_ctx = k_cache[:, kv_start:kv_total]
    v_ctx = v_cache[:, kv_start:kv_total]

    # Decode path (T==1) is naturally causal after cache slicing.
    if T == 1:
        block_mask = None
    elif precomputed_block_mask is not None and cache_seqlen == 0 and kv_start == 0:
        block_mask = precomputed_block_mask
    else:
        block_mask = _build_causal_window_mask(
            q_len=T,
            kv_len=int(k_ctx.size(1)),
            window_tokens=window_tokens,
            device=x.device,
            block_size=block_size,
            q_offset=cache_seqlen,
            kv_offset=kv_start,
        )

    score_mod = None
    if pos_rel_bias is not None:
        q_pos = pos_ids[:T].to(device=x.device, dtype=torch.int32)
        k_pos = pos_cache[:, kv_start:kv_total][0]
        score_mod = pos_rel_bias.build_score_mod(q_pos, k_pos)
...
def gpt_forward_infer_logits(
    ...
    *,
    ...
    pos_ids:   [T] POS ids used for relative bias (int-like tensor).
    Returns:
    ...
    assert T > 0
    if int(pos_ids.numel()) != T:
        raise ValueError(f"pos_ids length mismatch: expected {T}, got {int(pos_ids.numel())}")
    ...
    cache_seqlen = int(kv_cache.cache_seqlens[0].item())
    prefill_masks = model._get_infer_block_masks(T, ws_short, ws_long, input_seq.device) if (T > 1 and cache_seqlen == 0) else None
    ...
    for i, block in enumerate(model.transformer.h):
        ...
        x = x + _attn_forward_infer(
            ...
            k_cache=k_cache_i,
            v_cache=v_cache_i,
            pos_cache=kv_cache.pos_cache,
            cache_seqlen=cache_seqlen,
            precomputed_block_mask=(prefill_masks[i] if prefill_masks is not None else None),
            pos_rel_bias=model._get_pos_rpb_for_layer(i),
        )
        # MLP
...
def _run_inference_cli():
    ...
    # Dtype selection (FlexAttention kernels support bf16/fp16)
    if a.dtype == "bf16":
    ...
    with torch.no_grad(), autocast_ctx:
        cos, sin, pos_prompt = model.yarn(prompt, return_pos_ids=True)
    ...
    for _ in range(int(a.max_new_tokens)):
        ...
        cos1, sin1, pos1 = yarn_state.step(next_id)
    ...
    print(f"[infer] prompt_tokens={prompt_len} generated_total={len(generated)} use_kv_cache=True")
...
assert args.device_batch_size_tokens % 16 == 0, (
    "device_batch_size_tokens must be a multiple of 16 (attention kernel alignment requirement)"
)
...
def load_checkpoint(*, model: nn.Module, training_manager: TrainingManager, train_loader: ResumableDistributedDataLoader) -> int:
    ...
    if master_process:
        ...
        incompat = model_for_state.load_state_dict(model_state, strict=False)
        ...
        allow_prefixes = ("pos_rpb.", "pos_rpb_layers.")
        hard_missing = [k for k in missing_keys if not k.startswith(allow_prefixes)]
        hard_unexpected = [k for k in unexpected_keys if not k.startswith(allow_prefixes)]
        if hard_missing or hard_unexpected:
            raise RuntimeError(
                f"Checkpoint/model mismatch. missing={hard_missing} unexpected={hard_unexpected}"
            )
        if master_process and (missing_keys or unexpected_keys):
            print0(
                f"[resume] non-strict state_dict load: missing={missing_keys}, unexpected={unexpected_keys}",
                console=True,
            )
...
for step in range(start_step, train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        with torch.no_grad():
            for _ in range(val_steps):
                inputs, targets, cum_seqlens, _max_seqlen = next(val_loader)
                with autocast_ctx:
    ...
    for idx in range(training_manager.grad_accum_steps):
        ...
        inputs, targets, cum_seqlens, _max_seqlen = train_loader.send(send_args)
        with autocast_ctx:
