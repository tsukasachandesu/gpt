...
def _model_to_remi96_ids(tokens_model: Sequence[int], *, shift: int) -> list[int]:
    ...
    out = []
    shift = int(shift)
    thr = 96 + shift  # BAR id in model vocab when shift>0
...
def _run_inference_cli():
    ...
    parser.add_argument("--eof_id", type=int, default=130, help="EOF token id in model vocab (default 130).")
    parser.add_argument("--mask_unused_pitches", type=int, default=0, help="(互換用) vocab_size=132 では未使用 pitch トークン帯が存在しないため、通常は 0 のままで問題ありません。")
    ...
    if int(a.mask_unused_pitches) == 1:
        # 互換用: vocab_size=132 では未使用 pitch トークン帯が存在しないため no-op。
        pass
