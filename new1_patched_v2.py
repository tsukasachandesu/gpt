...
class Yarn(nn.Module):
    ...
    def reset(self):
        ...
        t_bar = torch.arange(self.max_seq_len + 1, dtype=torch.float32, device=dev)
        ...
        t_pitch = torch.arange(self.pitch_cache_len, dtype=torch.float32, device=dev)
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, num_kv_heads: int, head_dim: int, model_dim: int, max_seq_len: int):
        ...
        # --- REMI constraint masks (training-time loss masking) ---
        # Build once and keep as non-persistent buffers (deterministic; no checkpoint bloat).
        remi_masks = _build_remi_constraint_masks(
            vocab_size=vocab_size,
            device=torch.device("cpu"),
            remi=REMI96,
            eof_id=int(REMI96.eof_id),
        )
        self._remi_mask_pitch_lt = nn.Buffer(remi_masks["pitch_lt"], persistent=False)
        self._remi_mask_pos_gt = nn.Buffer(remi_masks["pos_gt"], persistent=False)
        self._remi_mask_bar = nn.Buffer(remi_masks["bar"], persistent=False)
        self._remi_mask_eof = nn.Buffer(remi_masks["eof"], persistent=False)
        self._remi_mask_pitch_all = nn.Buffer(remi_masks["pitch_all"], persistent=False)
        self._remi_mask_pos_all = nn.Buffer(remi_masks["pos_all"], persistent=False)
        self._remi_bar_eof = nn.Buffer(self._remi_mask_bar | self._remi_mask_eof, persistent=False)
        self._remi_state_const_mask = nn.Buffer(
            torch.stack([self._remi_mask_bar, self._remi_mask_pos_all, self._remi_mask_pitch_all], dim=0),
            persistent=False,
        )

        # Cache REMI constants as Python ints (torch.compile-friendly; no tensor overhead).
        self._remi_pitch_min = int(REMI96.pitch_min)
        self._remi_pitch_size = int(remi_masks["pitch_size"])
        self._remi_pos_offset = int(REMI96.pos_offset)
        self._remi_pos_size = int(remi_masks["pos_size"])
        self._remi_bar_id = int(REMI96.bar_id)
        self._remi_bos_id = int(REMI96.bos_id)
        self._remi_pad_id = int(REMI96.pad_id)
    ...
    def _remi_allowed_mask_for_loss(self, input_seq: Tensor, target_seq: Tensor) -> Tensor:
        """Vectorized REMI hard-constraint mask for next-token prediction loss.

        Returns:
            allowed: Bool tensor of shape (T, vocab_size). True means the token is allowed.
        """
        # State encoding (fixed, do not change without updating masks):
        #   0: after BOS
        #   1: after BAR
        #   2: after POS
        #   3: after PITCH
        ...
        tok = input_seq  # expected int32 (see data loader)
        T = tok.numel()
        idx = self.yarn._idx_cache[:T]  # (T,) int32

        pitch_min = self._remi_pitch_min
        pitch_size = self._remi_pitch_size
        pos_offset = self._remi_pos_offset
        pos_size = self._remi_pos_size

        # token types
        pitch_tok = (tok >= pitch_min) & (tok < (pitch_min + pitch_size))
        pos_tok = (tok >= pos_offset) & (tok < (pos_offset + pos_size))
        bar_tok = tok.eq(self._remi_bar_id)
        bos_tok = tok.eq(self._remi_bos_id)
        # PAD/EOF are treated as "carry" (no update) for state/last_pos/last_pitch.

        # last_pos: update on POS; reset on BAR/BOS; otherwise carry
        pos_updates = torch.where(pos_tok, tok - pos_offset, torch.zeros_like(tok))
        pos_updates = torch.where(bar_tok | bos_tok, tok.new_full((), -1).expand_as(tok), pos_updates)
        pos_update_mask = pos_tok | bar_tok | bos_tok
        last_pos = forward_fill_from_updates(pos_updates, pos_update_mask, idx)

        # last_pitch: update on PITCH; reset on POS/BAR/BOS; otherwise carry
        pitch_none = pitch_size
        pitch_updates = torch.where(
            pitch_tok,
            tok - pitch_min,
            tok.new_full((), pitch_none).expand_as(tok),
        )
        pitch_update_mask = pitch_tok | pos_tok | bar_tok | bos_tok
        last_pitch = forward_fill_from_updates(pitch_updates, pitch_update_mask, idx)

        # state: last non-(PAD/EOF) token type among {BOS, BAR, POS, PITCH}
        # BOS maps to STATE_AFTER_BOS (=0) and therefore does not need an explicit addend.
        state_updates = (
            bar_tok.to(dtype=torch.int32) * STATE_AFTER_BAR
            + pos_tok.to(dtype=torch.int32) * STATE_AFTER_POS
            + pitch_tok.to(dtype=torch.int32) * STATE_AFTER_PITCH
        )
        state_update_mask = bos_tok | bar_tok | pos_tok | pitch_tok
        state = forward_fill_from_updates(state_updates, state_update_mask, idx)

        # dynamic mask for STATE_AFTER_PITCH
        pos_row = (last_pos + 1).clamp(0, pos_size).to(torch.int64)  # [-1..] -> [0..pos_size]
        pitch_idx = last_pitch.clamp(0, pitch_size).to(torch.int64)  # [0..pitch_size], pitch_size means "None"

        pitch_mask = self._remi_mask_pitch_lt[pitch_idx]  # (T, V)
        pos_mask = self._remi_mask_pos_gt[pos_row]  # (T, V)

        # If last_pos is already at the maximum, disallow further POS (only pitch/bar/eof).
        pos_ok = (last_pos < (pos_size - 1)).unsqueeze(1)
        pitch_state_mask = pitch_mask | (pos_mask & pos_ok) | self._remi_bar_eof

        # constant masks for states 0..2
        const = self._remi_state_const_mask[state.clamp_max(STATE_AFTER_POS).to(torch.int64)]
        allowed = torch.where(state.eq(STATE_AFTER_PITCH).unsqueeze(1), pitch_state_mask, const)

        # Always allow the ground-truth target to avoid inf loss at packed-document boundaries (e.g., -> BOS).
        allowed.scatter_(1, target_seq.view(-1, 1), True)
        return allowed

    def _remi_state_mask_infer(self, state: int, last_pos: int, last_pitch: int | None) -> torch.Tensor:
        """Allowed-vocab mask for the *next* token under REMI constraints.

        This mirrors the semantics of `_remi_state_mask(...)` but uses the
        precomputed buffers stored on the model (no per-run mask building).

        Args:
            state: one of {0:AFTER_BOS, 1:AFTER_BAR, 2:AFTER_POS, 3:AFTER_PITCH}
            last_pos: last POS value within current bar, or -1 if none.
            last_pitch: last PITCH token id (absolute) in current POS, or None.
        """
        # Fast paths: constant masks.
        if state == 0:
            return self._remi_mask_bar
        if state == 1:
            return self._remi_mask_pos_all
        if state == 2:
            return self._remi_mask_pitch_all

        # AFTER_PITCH: dynamic pitch-descending + pos-increasing + {bar,eof}.
        pitch_size = self._remi_pitch_size
        pos_size = self._remi_pos_size

        if last_pitch is None or int(last_pitch) < 0:
            pitch_idx = pitch_size
        else:
            pitch_idx = int(last_pitch) - self._remi_pitch_min
            if pitch_idx < 0 or pitch_idx > pitch_size:
                pitch_idx = pitch_size

        pitch_mask = self._remi_mask_pitch_lt[pitch_idx]
        if int(last_pos) >= (pos_size - 1):
            return pitch_mask | self._remi_bar_eof

        pos_row = int(last_pos) + 1
        if pos_row < 0:
            pos_row = 0
        elif pos_row > pos_size:
            pos_row = pos_size
        pos_mask = self._remi_mask_pos_gt[pos_row]
        return pitch_mask | pos_mask | self._remi_bar_eof
    ...
    def forward(
        ...
        for i, block in enumerate(self.transformer.h):
            a = self.resid_lambdas[i].to(dtype=x.dtype)
            b = self.x0_lambdas[i].to(dtype=x.dtype)
            x = a * x + b * x0
        ...
        if not self.training:
            # Keep eval loss consistent with the training objective.
            allowed_full = self._remi_allowed_mask_for_loss(input_seq, target_seq)
            ...
            for x_chunk, t_chunk in zip(x_chunks, t_chunks):
                ...
                logits = logits.masked_fill(~allowed_full[offset : offset + n], float("-inf"))
        ...
        logits = logits.float()
        logits = softcap * torch.tanh(logits / softcap)
        # Training-time REMI constraint masking (vectorized; torch.compile-friendly).
        allowed = self._remi_allowed_mask_for_loss(input_seq, target_seq)
        logits_flat = logits.view(-1, logits.size(-1))
        logits_flat = logits_flat.masked_fill(~allowed, float("-inf"))
...
def gpt_forward_infer_logits(
    ...
    *,
    ...
    for i, block in enumerate(model.transformer.h):
        a = model.resid_lambdas[i].to(dtype=x.dtype)
        b = model.x0_lambdas[i].to(dtype=x.dtype)
        x = a * x + b * x0
...
def _run_inference_cli():
    ...
    if enforce_remi:
        # Initialize state from prompt tokens; per-step masking is handled by the model's
        # precomputed REMI buffers (no need to rebuild masks in the CLI).
    ...
    if use_kv_cache:
        ...
        for _ in range(int(a.max_new_tokens)):
            ...
            if enforce_remi:
                state_mask = model._remi_state_mask_infer(remi_state, remi_last_pos, remi_last_pitch)
