...
class GPTConfig:
    ...
    fme_mode : str = "replace"  # "replace" or "add"
    pitch_start : int = 0
    pitch_size : int = 128
    bar_start : int = 128
    bar_size : int = 1
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.use_fme:
            self.fme_pitch = FundamentalMusicEmbedding(config.n_embd, base=config.fme_base)
            self.fme_bar = FundamentalMusicEmbedding(config.n_embd, base=config.fme_base)
            self.fme_pos = FundamentalMusicEmbedding(config.n_embd, base=config.fme_base)
    ...
    def forward(self, idx, target, attn_blocksize):
        ...
        if self.config.use_fme:
            b, t, c = x.shape
            idx_flat = idx.reshape(-1)
            emb_flat = x.reshape(-1, c)

            def apply_fme(start, size, fme_module):
                end = start + size
                mask = (idx_flat >= start) & (idx_flat < end)
                if mask.any():
                    values = idx_flat[mask] - start
                    fme_emb = fme_module(values).to(dtype=emb_flat.dtype)
                    if self.config.fme_mode == "add":
                        emb_flat[mask] = emb_flat[mask] + fme_emb
                    else:
                        emb_flat[mask] = fme_emb

            apply_fme(self.config.pitch_start, self.config.pitch_size, self.fme_pitch)
            apply_fme(self.config.bar_start, self.config.bar_size, self.fme_bar)
            apply_fme(self.config.pos_start, self.config.pos_size, self.fme_pos)
            x = emb_flat.view(b, t, c)
