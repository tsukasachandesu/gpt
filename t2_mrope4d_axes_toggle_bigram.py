...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, num_kv_heads: int, head_dim: int, model_dim: int, max_seq_len: int):
        ...
        # Bigram hash embedding (PR #201): hashed bigrams looked up once per token and added before every layer
        assert args.bigram_vocab_size >= 2, "args.bigram_vocab_size must be >= 2"
        self.bigram_embed = nn.Embedding(args.bigram_vocab_size, model_dim)

        # Per-layer scalars
        self.resid_lambdas = nn.Parameter(torch.ones(num_layers))
        self.x0_lambdas = nn.Parameter(torch.zeros(num_layers))
        self.bigram_lambdas = nn.Parameter(0.1 * torch.ones(num_layers))
    ...
    def init_weights(self):
        ...
        # Zero-init hashed bigram embedding table so learning is driven by nonzero bigram_lambdas
    ...
    def setup_optimizers(
        ...
        # Token embedding + hashed bigram embedding share the same optimizer hyperparameters
        embedding_params = list(self.transformer.wte.parameters()) + list(self.bigram_embed.parameters())
        lm_head_params = list(self.lm_head.parameters())
        # Per-layer scalars: residual decay + bigram mixing (slow LR), and x0 mixing (faster LR)
...
def get_bigram_hash(x: Tensor) -> Tensor:
    """
    Computes bigram hash for each position using [prev_token, curr_token].
    Multiply by arbitrary large ints to get even spread over int32 range.
    Position 0 is mapped to the reserved index (bigram_vocab_size - 1).
    BOS tokens within the batch will hash based on last token of prior doc.
    Masking this ran slower and showed no improvement (per modded-nanogpt PR #201).
    """
    rand_int_1 = 36313
    rand_int_2 = 27191
...
class Hyperparameters:
    ...
    # - reserved index: bigram_vocab_size - 1
    # Setting this to 0 will auto-set to 5 * padded_vocab_size.
    bigram_vocab_size: int = 0

args = Hyperparameters()

# Default bigram vocab size: 5x padded vocab size (matching modded-nanogpt PR #201)
if args.bigram_vocab_size == 0:
    args.bigram_vocab_size = 5 * next_multiple_of_n(args.vocab_size, n=64)
assert args.bigram_vocab_size >= 2, "bigram_vocab_size must be >= 2"
...
if model.transformer.wte.weight.device.type == "cuda":
    ...
    model.bigram_embed.to(dtype=torch.bfloat16)
...
for step in range(train_steps + 1):
    ...
    for idx in range(training_manager.grad_accum_steps):
        ...
        with autocast_ctx:
            (model(inputs, targets, cum_seqlens, bigram_inputs, training_manager.get_forward_args()) / training_manager.grad_accum_steps).backward()
