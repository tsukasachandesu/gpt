...
class Yarn(nn.Module):
    ...
    pitch について:
      * pitch 軸は「生 pitch」(0..pitch_size-1) の回転に加え、
        12階音（pitch class）とオクターブ（octave）に分解した回転を使用できる。
      * pitch_pc_oct=True の場合、pitch に割り当てた rotary pairs を
        pitch class / octave に分割して上書きする（軸数は増やさず、pitch 内で分割）。
    ...
    注意:
      ...
      * pitch_pc_oct=True 時は enable_pitch_class/enable_pitch_octave で pitch 内の分解成分も ON/OFF 可能
    """
    ...
    def __init__(
        ...
        *,
        ...
        # pitch: pitch class / octave 分解
        pitch_pc_oct: bool = True,
        pitch_class_mod: int = 12,
        pitch_pc_oct_fractions: tuple[int, int] = (1, 1),
        enable_pitch_class: bool = True,
        enable_pitch_octave: bool = True,
    ):
        super().__init__()
        if head_dim % 2 != 0:
        ...
        # pitch split enables
        self.pitch_pc_oct = bool(pitch_pc_oct)
        self.pitch_class_mod = int(pitch_class_mod)
        if self.pitch_class_mod <= 0:
            raise ValueError(f"pitch_class_mod must be positive, got {pitch_class_mod}")

        self.enable_pitch_class = bool(enable_pitch_class)
        self.enable_pitch_octave = bool(enable_pitch_octave)

        # cache lengths for pitch class / octave (0 reserved as 'none' via pitch_idx_offset)
        self.pitch_class_cache_len = int(self.pitch_class_mod + self.pitch_idx_offset)
        pitch_octaves = (self.pitch_size + self.pitch_class_mod - 1) // self.pitch_class_mod
        self.pitch_octave_cache_len = int(pitch_octaves + self.pitch_idx_offset)
        ...
        # split pitch pairs into pitch class / octave (only used if pitch_pc_oct=True)
        if self.pitch_pc_oct and self.pairs_pitch > 0:
            self.pairs_pitch_class, self.pairs_pitch_octave = self._allocate_pairs_2(
                int(self.pairs_pitch), pitch_pc_oct_fractions
            )
        else:
            self.pairs_pitch_class = 0
            self.pairs_pitch_octave = 0
    ...
    @staticmethod
    def _allocate_pairs_2(total_pairs: int, ratios: tuple[int, int]) -> tuple[int, int]:
        r0, r1 = (int(r) for r in ratios)
        if min(r0, r1) <= 0:
            raise ValueError(f"pitch_pc_oct_fractions must be positive, got {ratios}")
        denom = r0 + r1
        scaled0 = total_pairs * r0
        scaled1 = total_pairs * r1
        a0 = scaled0 // denom
        a1 = scaled1 // denom
        rem = total_pairs - (a0 + a1)
        # allocate remainder to the larger fractional part; tie -> class first
        if rem > 0:
            frac0 = scaled0 % denom
            frac1 = scaled1 % denom
            if frac1 > frac0:
    ...
    def set_axes(
        ...
        *,
        ...
        pitch_class: bool | None = None,
        pitch_octave: bool | None = None,
    ) -> None:
        ...
        if pitch_class is not None:
            self.enable_pitch_class = bool(pitch_class)
        if pitch_octave is not None:
            self.enable_pitch_octave = bool(pitch_octave)
    ...
    def reset(self):
        ...
        # pitch caches:
        #  - raw pitch (when pitch_pc_oct=False): (pitch_cache_len, total_pairs)
        #  - pitch class / octave (when pitch_pc_oct=True): compact tables (cache_len, pairs_pitch_{class,octave})
        if self.pitch_pc_oct:
            # Determine which inv_freq columns correspond to pitch-axis slots.
            if self.pair_layout == 'block':
                s0, s1, s2, _s3 = self.mrope_section
                o3 = int(s0 + s1 + s2)
                pc_pairs = int(self.pairs_pitch_class)
                oc_pairs = int(self.pairs_pitch_octave)
                inv_pc = inv_freq[o3:o3 + pc_pairs]
                inv_oc = inv_freq[o3 + pc_pairs:o3 + pc_pairs + oc_pairs]
            else:
                stride = 4
                pc_pairs = int(self.pairs_pitch_class)
                oc_pairs = int(self.pairs_pitch_octave)
                if pc_pairs > 0:
                    idx_pc = torch.arange(pc_pairs, device=device, dtype=torch.int64) * stride + 3
                    inv_pc = inv_freq.index_select(0, idx_pc)
                else:
                    inv_pc = inv_freq[:0]
                if oc_pairs > 0:
                    idx_oc = torch.arange(oc_pairs, device=device, dtype=torch.int64) * stride + (3 + stride * pc_pairs)
                    inv_oc = inv_freq.index_select(0, idx_oc)
                else:
                    inv_oc = inv_freq[:0]

            # pitch class
            t_pc = torch.arange(self.pitch_class_cache_len, dtype=torch.float32, device=device)
            theta_pc = torch.outer(t_pc, inv_pc)
            self._cos_pitch_pc = nn.Buffer(theta_pc.cos().to(torch.bfloat16), persistent=False)
            self._sin_pitch_pc = nn.Buffer(theta_pc.sin().to(torch.bfloat16), persistent=False)

            # pitch octave
            t_oc = torch.arange(self.pitch_octave_cache_len, dtype=torch.float32, device=device)
            theta_oc = torch.outer(t_oc, inv_oc)
            self._cos_pitch_oct = nn.Buffer(theta_oc.cos().to(torch.bfloat16), persistent=False)
            self._sin_pitch_oct = nn.Buffer(theta_oc.sin().to(torch.bfloat16), persistent=False)
    ...
    def forward(self, token_ids: Tensor) -> tuple[Tensor, Tensor]:
        ...
        if self.pitch_pc_oct:
            use_pitch_pc = self.enable_pitch and self.enable_pitch_class and (self.pairs_pitch_class > 0)
            use_pitch_oct = self.enable_pitch and self.enable_pitch_octave and (self.pairs_pitch_octave > 0)
            use_pitch_raw = False
        else:
            use_pitch_raw = self.enable_pitch and (self.pairs_pitch > 0)
            use_pitch_pc = False
            use_pitch_oct = False

        need_extra = use_bar or use_pos or use_pitch_raw or use_pitch_pc or use_pitch_oct
        ...
        # masks (only comparisons; compile can DCE unused branches)
        ...
        # gather cos/sin per enabled axis (shapes depend on axis)
        ...
        # pitch indices (shared pre-processing)
        pitch_idx = None
        if use_pitch_raw or use_pitch_pc or use_pitch_oct:
        ...
        if self.pair_layout == 'block':
            ...
            if use_pitch_pc or use_pitch_oct:
                # derive pitch class / octave indices from pitch_idx (0 is 'none')
                pitch_rel = (pitch_idx - self.pitch_idx_offset).clamp_min(0)
                pitch_nonzero = pitch_idx != 0

                pc_idx = torch.remainder(pitch_rel, self.pitch_class_mod) + self.pitch_idx_offset
                oc_idx = (pitch_rel // self.pitch_class_mod) + self.pitch_idx_offset
                pc_idx = pc_idx.masked_fill(~pitch_nonzero, 0).clamp(0, self.pitch_class_cache_len - 1)
                oc_idx = oc_idx.masked_fill(~pitch_nonzero, 0).clamp(0, self.pitch_octave_cache_len - 1)

                pc_pairs = int(self.pairs_pitch_class)
                oc_pairs = int(self.pairs_pitch_octave)
                if use_pitch_pc and pc_pairs > 0:
                    cos_pc = F.embedding(pc_idx, self._cos_pitch_pc)
                    sin_pc = F.embedding(pc_idx, self._sin_pitch_pc)
                    cos[:, o3:o3 + pc_pairs] = cos_pc
                    sin[:, o3:o3 + pc_pairs] = sin_pc
                if use_pitch_oct and oc_pairs > 0:
                    cos_oc = F.embedding(oc_idx, self._cos_pitch_oct)
                    sin_oc = F.embedding(oc_idx, self._sin_pitch_oct)
                    cos[:, o3 + pc_pairs:o3 + pc_pairs + oc_pairs] = cos_oc
                    sin[:, o3 + pc_pairs:o3 + pc_pairs + oc_pairs] = sin_oc
            ...
            if use_pitch_pc or use_pitch_oct:
                ...
                if use_pitch_pc and pc_pairs > 0:
                    ...
                    cos[:, 3:end:stride] = cos_pc
                    sin[:, 3:end:stride] = sin_pc

                if use_pitch_oct and oc_pairs > 0:
                    cos_oc = F.embedding(oc_idx, self._cos_pitch_oct)
                    sin_oc = F.embedding(oc_idx, self._sin_pitch_oct)
                    start = 3 + pc_pairs * stride
                    end = (pc_pairs + oc_pairs) * stride
                    cos[:, start:end:stride] = cos_oc
                    sin[:, start:end:stride] = sin_oc
