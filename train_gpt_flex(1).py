...
# -----------------------------------------------------------------------------
# FlexAttention varlen wrapper (replacement for flash_attn_varlen_func)
#
# Notes on correctness & performance:
# - flex_attention expects (B, H, L, D). We express varlen sequences via NestedTensor (jagged layout).
# - create_nested_block_mask packs nested sequences and enforces no inter-sequence attention automatically.
# - mask_mod must be stable across calls to avoid repeated compilation overhead.

@lru_cache(maxsize=None)
def _flex_mask_causal():
    def mask_mod(b, h, q_idx, kv_idx):
        return q_idx >= kv_idx
    return mask_mod

@lru_cache(maxsize=None)
def _flex_mask_causal_window(window_size: int):
    # window_size: left window size (tokens); matches flash-attn window_size=(window_size, 0) with causal=True.
    def mask_mod(b, h, q_idx, kv_idx):
...
def flex_attention_varlen(
    ...
    *,
    ...
    """Varlen + sliding-window causal attention via FlexAttention.

    Drop-in replacement for the original flash-attn varlen call:
    ...
    Args:
      q,k,v: (T, H, D)
      seqlens: cu_seqlens offsets (shape: num_seqs+1, may be padded with repeats of T)
      max_len: max per-sequence length
      bm_size: left window size (tokens)

    Returns:
      (T, H, D)
    """
    # torch.nested expects offsets int64.
    offsets = seqlens.to(dtype=torch.int64)

    # NJT layout from packed (T, H, D) + offsets. After transpose: (B, H, L*, D)
    q_nt = torch.nested.nested_tensor_from_jagged(values=q, offsets=offsets, max_seqlen=max_len).transpose(1, 2)
    k_nt = torch.nested.nested_tensor_from_jagged(values=k, offsets=offsets, max_seqlen=max_len).transpose(1, 2)
    v_nt = torch.nested.nested_tensor_from_jagged(values=v, offsets=offsets, max_seqlen=max_len).transpose(1, 2)

    # Build a causal (+ optional sliding window) mask_mod.
    if bm_size is None or bm_size <= 0:
        mask_mod = _flex_mask_causal()
    else:
        mask_mod = _flex_mask_causal_window(int(bm_size))

    # BlockMask generation is compiled (once per unique mask_mod / shape).
    ...
    out_nt = flex_attention(q_nt, k_nt, v_nt, block_mask=block_mask, scale=softmax_scale)

    # Return packed values with original (T, H, D) layout
    return out_nt.transpose(1, 2).values()
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, attn_args: AttnArgs):
        ...
        # FlexAttention (varlen via NJT + BlockMask)
        y = flex_attention_varlen(
            q[0], k[0], v[0],
            seqlens=seqlens,
            max_len=max_len,
            bm_size=bm_size,
