...
# 4軸（index / bar / pos / pitch）のMRoPE（Interleaved-MRoPE互換）を内包する YaRN
...
class Yarn(nn.Module):
    """YaRN + 4軸MRoPE（index / bar / pos / pitch）.

    互換性目標:
      * Qwen 系（Qwen2-VL / Qwen3-VL）で使われる Interleaved-MRoPE の「固定ストライド上書き」を 4軸に拡張。
        vLLM の `apply_interleaved_rope` 相当（3軸）を、stride=4 で一般化する。
    ...
    注意:
      * 本実装は "chunked RoPE"（head_dim を前半/後半に split する方式）を維持する。
      * half-truncate RoPE（後半の周波数を 0 にして回転を止める）も維持する。
    """
    ...
    def __init__(
        ...
        *,
        ...
        # ペア並び: 'interleave'（Qwen3-VL互換） or 'block'
        ...
        self.pairs_index, self.pairs_bar, self.pairs_pos, self.pairs_pitch = self._allocate_pairs(total_pairs, axial_fractions)
        ...
        # Validate interleaved fixed-stride feasibility
    ...
    def reset(self):
        # base inv_freq (half-truncate RoPE by @YouJiacheng)
        ...
        # cos/sin caches per axis (all axes share the same inv_freq; axisごとに position id だけを変える)
        # index: [0..T)
        ...
        theta_bar = torch.outer(t_bar, angular_freq)
        ...
        theta_pos = torch.outer(t_pos, angular_freq)
        ...
        theta_pitch = torch.outer(t_pitch, angular_freq)
    ...
    def apply(self, old_window: int, new_window: int, alpha: int = 1, beta: int = 32):
        ...
        self._cos_index.copy_(theta.cos())
        self._sin_index.copy_(theta.sin())

        # rebuild bar cache
        t_bar = torch.arange(self.max_seq_len + 1, dtype=torch.float32, device=self.angular_freq.device)
        theta_bar = torch.outer(t_bar, self.angular_freq)
        self._cos_bar.copy_(theta_bar.cos())
        self._sin_bar.copy_(theta_bar.sin())
    ...
    def forward(self, token_ids: Tensor) -> tuple[Tensor, Tensor]:
        ...
        # gather cos/sin per axis (shape: [T, total_pairs])
        ...
        total_pairs = self.head_dim // 2
        if cos_index.size(-1) != total_pairs:
            raise RuntimeError('Internal error: cache last-dim mismatch')
        ...
        # interleaved fixed-stride overwrite (Qwen3-VL style)
        cos = cos_index.clone()
        sin = sin_index.clone()
        stride = 4
        sec_bar = int(self.pairs_bar)
        sec_pos = int(self.pairs_pos)
        sec_pitch = int(self.pairs_pitch)
        if sec_bar > 0:
            end = sec_bar * stride
...
class GPT(nn.Module):
    ...
    def create_blockmasks(self, input_seq: Tensor, ws_long: int, ws_short: int):
        ...
        self._mask_mod_long.set_doc_ids(doc_ids)
        self._mask_mod_short.set_doc_ids(doc_ids)
        self._mask_mod_long.set_window(ws_long * BLOCK_SIZE)
        self._mask_mod_short.set_window(ws_short * BLOCK_SIZE)


        # Fast path: if long == short, build once and reuse to avoid extra overhead.
        if ws_long == ws_short:
            bm_mask = _build_doc_causal_window_blockmask(
                doc_ids,
                window_blocks=ws_long,
                block_size=BLOCK_SIZE,
                mask_mod=self._mask_mod_long,
            )
            return bm_mask, bm_mask
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, seqlens: Tensor, schedule_cfg: ForwardScheduleConfig):
        ...
        long_bm_mask, short_bm_mask = self.create_blockmasks(input_seq, ws_long, ws_short)
        block_masks = [long_bm_mask if (bm == long_bm) else short_bm_mask for bm in bm_sizes]

        # 4軸MRoPE: トークン列から cos/sin を一度だけ生成し、全層で共有
