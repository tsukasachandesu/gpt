...
def _fa_doc_causal_window_mask_impl(
    *,
    ...
    # NOTE: Avoid torch.clamp(..., max=<0-dim Tensor>) here. During torch.compile
    # tracing, FakeTensor/meta implementations may attempt to materialize the
    # 0-dim max value as a Python scalar (aten._local_scalar_dense), which is
    # rejected under fullgraph=True as data-dependent.
    max_idx = doc_len_i - 1
    max_idx = torch.maximum(max_idx, torch.zeros_like(max_idx))
    q_i = torch.minimum(torch.maximum(q_idx, torch.zeros_like(q_idx)), max_idx)
    kv_i = torch.minimum(torch.maximum(kv_idx, torch.zeros_like(kv_idx)), max_idx)

    # IMPORTANT for torch.compile(fullgraph=True):
    # Avoid Python-level indexing like `buf[q_i]` when q_i is a 0-dim Tensor.
    # Depending on the PyTorch build, this can implicitly materialize a Python
    # scalar via aten._local_scalar_dense (i.e. `.item()`), which Dynamo rejects
    # as data-dependent. Use aten.index explicitly to keep everything tensorized.
    doc_q = torch.ops.aten.index.Tensor(doc_id_buf, [q_i])
    doc_kv = torch.ops.aten.index.Tensor(doc_id_buf, [kv_i])
...
# Register ForwardScheduleConfig as a pytree node so torch.compile can
# treat its Tensor members (and nested BlockMask objects) as graph inputs
# rather than opaque Python objects. This helps fullgraph=True stability and
# avoids recompilations when new schedule objects are created each step.
def _register_forward_schedule_config_pytree() -> None:
    try:
        def _flatten(cfg: ForwardScheduleConfig):
            children = (
                cfg.mtp_weights,
                cfg.ws_short,
                cfg.ws_long,
        ...
        register_pytree_node(ForwardScheduleConfig, _flatten, _unflatten)
    except Exception:
        # Best effort; torch versions differ in pytree APIs.
        pass


_register_forward_schedule_config_pytree()
