...
# -----------------------------------------------------------------------------
# 4D Axial MRoPE (index / bar / pos / pitch)
# - Interleaved-MRoPE is compatible with Qwen3-VL's "apply_interleaved_mrope".
# - This replaces the previous 1D half-truncated RoPE inside Yarn.
# - YaRN-style window extension (apply) is preserved, and applied to index+bar axes.
# -----------------------------------------------------------------------------

def forward_fill_from_updates(update_values: torch.Tensor, update_mask: torch.Tensor) -> torch.Tensor:
    """Forward-fill (LOCF) the latest update values along the sequence dimension.

    Args:
        update_values: (B, T) values at positions where update_mask is True.
        update_mask:   (B, T) bool mask.

    Returns:
        (B, T) forward-filled values; positions before the first update are 0.
    """
    # NOTE: Keep this function purely tensorized (torch.compile friendly).
    update_mask = update_mask.to(dtype=torch.bool)
    B, T = update_values.shape
    device_ = update_values.device

    t = torch.arange(T, device=device_, dtype=torch.long).view(1, T).expand(B, T)
...
def apply_rotary_emb(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
    ...
    Layout: pairs are (x[..., i], x[..., i + head_dim/2]).

    Args:
        x:   (B, T, H, D)
        cos: (B, T, 1, D/2) or broadcastable
        sin: (B, T, 1, D/2) or broadcastable
...
class _MRoPE4DCache(nn.Module):
    """Shared 4-axis MRoPE cache builder.

    This module precomputes per-axis cos/sin tables and exposes utilities to
    assemble the final Interleaved-MRoPE cos/sin for a given token sequence.

    YaRN-style apply(old_window, new_window) is supported and updates the
    (index, bar) axis frequencies + their caches.
    """
    ...
    def __init__(
        ...
        *,
        ...
        pos_size: int = 32,
        bar_id: int = 128,
        doc_id: int = 161,  # BOS in this codebase
        # index offsets to reserve 0 as "axis disabled"
        ...
        self.total_pairs = self.head_dim // 2
        self.max_seq_len = int(max_seq_len)
        self.base = int(base)
        ...
        if pair_layout not in ("interleave", "block"):
            raise ValueError(f"pair_layout must be 'interleave' or 'block', got {pair_layout!r}")
        self.pair_layout = pair_layout

        self.half_truncate = bool(half_truncate)
        self.mrope_section = self._allocate_pairs(self.total_pairs, axial_fractions)
        if self.pair_layout == "interleave":
            # Fixed-stride interleave needs each secondary axis section to fit.
            num_axes = 4
            for axis, sec in enumerate(self.mrope_section[1:], start=1):
                sec = int(sec)
                if sec <= 0:
                    continue
                max_needed = axis + (sec - 1) * num_axes
                if max_needed >= self.total_pairs:
                    raise ValueError(
                        f"Invalid mrope_section={self.mrope_section} for total_pairs={self.total_pairs} (axis={axis})"
                    )

        # Per-axis inverse frequencies (fp32 buffers).
        theta = self._build_theta(self.head_dim, base=self.base).to(device=device, dtype=torch.float32)
        if self.half_truncate:
            theta = theta.clone()
            theta[self.total_pairs // 2 :] = 0
        self.theta_base = nn.Buffer(theta.clone(), persistent=False)
        self.theta_index = nn.Buffer(theta.clone(), persistent=False)
        self.theta_bar = nn.Buffer(theta.clone(), persistent=False)
        self.theta_pos = nn.Buffer(theta.clone(), persistent=False)
        self.theta_pitch = nn.Buffer(theta.clone(), persistent=False)

        # Cache lengths
        self.index_cache_len = int(2 * self.max_seq_len)
        self.bar_cache_len = int(2 * self.max_seq_len + 1)

        # Precompute per-axis cos/sin caches (bf16) on the model device.
        cos_i, sin_i = self._build_cache(self.theta_index, self.index_cache_len)
        cos_b, sin_b = self._build_cache(self.theta_bar, self.bar_cache_len)
        cos_p, sin_p = self._build_cache(self.theta_pos, self.pos_cache_len)
        cos_pi, sin_pi = self._build_cache(self.theta_pitch, self.pitch_cache_len)

        self._cos_index = nn.Buffer(cos_i, persistent=False)
        self._sin_index = nn.Buffer(sin_i, persistent=False)
        self._cos_bar = nn.Buffer(cos_b, persistent=False)
        self._sin_bar = nn.Buffer(sin_b, persistent=False)
        self._cos_pos = nn.Buffer(cos_p, persistent=False)
        self._sin_pos = nn.Buffer(sin_p, persistent=False)
        self._cos_pitch = nn.Buffer(cos_pi, persistent=False)
        self._sin_pitch = nn.Buffer(sin_pi, persistent=False)
    ...
    def _allocate_pairs(total_pairs: int, ratios: tuple[int, int, int, int]) -> tuple[int, int, int, int]:
        ...
        denom = r0 + r1 + r2 + r3
        if denom <= 0:
            raise ValueError("axial_fractions must be positive")
        ...
        for i in order[:rem]:
            base[i] += 1
        # ensure each axis gets at least 1 pair
        if any(v <= 0 for v in base):
            raise ValueError(f"axial_fractions={ratios} allocates non-positive pairs: {tuple(base)}")
        return int(base[0]), int(base[1]), int(base[2]), int(base[3])
    ...
    def _build_cache(self, theta: torch.Tensor, max_len: int) -> tuple[torch.Tensor, torch.Tensor]:
        # Compute trig in fp32, then cast to bf16 once (avoid per-step casts).
        seq = torch.arange(max_len, device=theta.device, dtype=torch.float32)
        freqs = torch.outer(seq, theta.to(dtype=torch.float32))
        return freqs.cos().to(dtype=torch.bfloat16), freqs.sin().to(dtype=torch.bfloat16)

    def _combine_axes(self, x_axes: torch.Tensor) -> torch.Tensor:
        """Combine (4,B,T,total_pairs) into (B,T,total_pairs) based on pair_layout."""
        if self.pair_layout == "block":
            # [axis0... axis1... axis2... axis3...]
            s0, s1, s2, s3 = self.mrope_section
            splits = x_axes.split((s0, s1, s2, s3), dim=-1)
            return torch.cat([seg[i] for i, seg in enumerate(splits)], dim=-1)

        # Interleaved (fixed stride): overwrite axis1..3 at indices {i, i+4, i+8, ...}.
        out = x_axes[0].clone()
        num_axes = 4
        sec1, sec2, sec3 = self.mrope_section[1], self.mrope_section[2], self.mrope_section[3]
        if sec1 > 0:
    ...
    def _axis_indices(self, token_ids_2d: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Compute per-token indices for (bar, pos, pitch) axes.

        Returns:
            bar_idx, pos_idx, pitch_idx, pitch_tok
        """
        ...
        bar_tok = token_ids.eq(self.bar_id)
        doc_tok = token_ids.eq(self.doc_id)

        bar_idx = torch.cumsum(bar_tok.to(dtype=torch.long), dim=1)
        # Reset bar count on doc boundary (documents are already masked, but this keeps semantics clean).
        ...
        # Clamp to cache range for safety.
        ...
        return bar_idx.to(dtype=torch.long), pos_idx.to(dtype=torch.long), pitch_idx.to(dtype=torch.long), pitch_tok

    def build_cos_sin(self, token_ids: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Build (cos, sin) for a non-paired sequence.

        Args:
            token_ids: (T,) or (B,T)

        Returns:
            cos, sin: (B,T,total_pairs)
        """
        if token_ids.ndim == 1:
        ...
        # Axis indices
        bar_idx, pos_idx, pitch_idx, _pitch_tok = self._axis_indices(token_ids)

        # Gather per-axis cos/sin (all axes are full total_pairs).
        cos_index = self._cos_index[:T].unsqueeze(0).expand(B, -1, -1)
        sin_index = self._sin_index[:T].unsqueeze(0).expand(B, -1, -1)
        ...
        cos_axes = torch.stack((cos_index, cos_bar, cos_pos, cos_pitch), dim=0)
        sin_axes = torch.stack((sin_index, sin_bar, sin_pos, sin_pitch), dim=0)

        cos = self._combine_axes(cos_axes)
        sin = self._combine_axes(sin_axes)
        return cos, sin

    def build_cos_sin_even_odd(self, token_ids: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Build (cos_even, sin_even, cos_odd, sin_odd) for paired-head attention.

        The paired-head attention interleaves *heads* into a 2T sequence. We treat
        the "index" axis as the interleaved position (0..2T-1), while bar/pos/pitch
        are computed on the original token stream and shared by even/odd.

        Returns:
            cos_even/sin_even/cos_odd/sin_odd: (B,T,total_pairs)
        """
        if token_ids.ndim == 1:
            token_ids = token_ids.unsqueeze(0)
        B, T = token_ids.shape
        ...
        # index positions for interleaved 2T sequence
        twoT = int(2 * T)
        # (twoT, total_pairs)
        cos_index_2t = self._cos_index[:twoT]
        sin_index_2t = self._sin_index[:twoT]
        cos_index_even = cos_index_2t[0:twoT:2].unsqueeze(0).expand(B, -1, -1)
        sin_index_even = sin_index_2t[0:twoT:2].unsqueeze(0).expand(B, -1, -1)
        cos_index_odd = cos_index_2t[1:twoT:2].unsqueeze(0).expand(B, -1, -1)
        sin_index_odd = sin_index_2t[1:twoT:2].unsqueeze(0).expand(B, -1, -1)

        # bar/pos/pitch are shared for even/odd (same token id duplicated).
        cos_bar = F.embedding(bar_idx, self._cos_bar)
        sin_bar = F.embedding(bar_idx, self._sin_bar)
        cos_pos = F.embedding(pos_idx, self._cos_pos)
        sin_pos = F.embedding(pos_idx, self._sin_pos)
        ...
        # Assemble even
        cos_axes_even = torch.stack((cos_index_even, cos_bar, cos_pos, cos_pitch), dim=0)
        sin_axes_even = torch.stack((sin_index_even, sin_bar, sin_pos, sin_pitch), dim=0)
        cos_even = self._combine_axes(cos_axes_even)
        sin_even = self._combine_axes(sin_axes_even)

        # Assemble odd
        cos_axes_odd = torch.stack((cos_index_odd, cos_bar, cos_pos, cos_pitch), dim=0)
        sin_axes_odd = torch.stack((sin_index_odd, sin_bar, sin_pos, sin_pitch), dim=0)
        cos_odd = self._combine_axes(cos_axes_odd)
        sin_odd = self._combine_axes(sin_axes_odd)

        return cos_even, sin_even, cos_odd, sin_odd

    def apply(self, old_window: int, new_window: int, alpha: int = 1, beta: int = 32):
        """YaRN-style window extension.

        This preserves the previous behavior (frequency interpolation + attention scale update),
        but now applies to (index, bar) axes of MRoPE.
        """
        # Update inverse frequencies for index axis.
        scaling_factor = float(old_window) / float(new_window)

        def _yarn_update(theta: torch.Tensor) -> torch.Tensor:
            rotations = args.block_size * float(old_window) * theta / (2.0 * torch.pi)
            interpolation_weight = torch.clamp((rotations - float(alpha)) / float(beta - alpha), 0.0, 1.0)
            return theta * (scaling_factor + interpolation_weight * (1.0 - scaling_factor))

        with torch.no_grad():
            self.theta_index.copy_(_yarn_update(self.theta_index))
            self.theta_bar.copy_(_yarn_update(self.theta_bar))

            cos_i, sin_i = self._build_cache(self.theta_index, self.index_cache_len)
            cos_b, sin_b = self._build_cache(self.theta_bar, self.bar_cache_len)
            self._cos_index.copy_(cos_i)
            self._sin_index.copy_(sin_i)
            self._cos_bar.copy_(cos_b)
            self._sin_bar.copy_(sin_b)


    def reset(self):
        """Reset MRoPE caches to the unscaled base frequencies."""
        with torch.no_grad():
            self.theta_index.copy_(self.theta_base)
            self.theta_bar.copy_(self.theta_base)
            self.theta_pos.copy_(self.theta_base)
            self.theta_pitch.copy_(self.theta_base)
            ...
            self._cos_bar.copy_(cos_b)
            self._sin_bar.copy_(sin_b)
            self._cos_pos.copy_(cos_p)
            self._sin_pos.copy_(sin_p)
            self._cos_pitch.copy_(cos_pi)
            self._sin_pitch.copy_(sin_pi)


class Yarn(nn.Module):
    """YaRN + 4-axis Interleaved-MRoPE (index / bar / pos / pitch)."""
    ...
    def __init__(self, head_dim: int, max_seq_len: int):
        ...
        self.mrope = _MRoPE4DCache(head_dim=self.head_dim, max_seq_len=self.max_seq_len, pair_layout="interleave")

        # Per-forward cos/sin buffers for the *current* (non-paired) sequence.
        self._cos = nn.Buffer(
            torch.empty(self.mrope.index_cache_len, self.mrope.total_pairs, device=device, dtype=torch.bfloat16),
            persistent=False,
        )
        self._sin = nn.Buffer(
            torch.empty(self.mrope.index_cache_len, self.mrope.total_pairs, device=device, dtype=torch.bfloat16),
        ...
        # start with 0.1, preserved from the original code
        self.attn_scale = 0.1

    def set_token_ids(self, token_ids: torch.Tensor) -> None:
        """Precompute cos/sin for the given token stream (once per forward)."""
        cos, sin = self.mrope.build_cos_sin(token_ids)
        # cos/sin: (B,T,total_pairs). This codebase enforces B==1.
        T = cos.size(1)
        self._cos[:T].copy_(cos[0])
        self._sin[:T].copy_(sin[0])

    def rotary(self, x_BTHD: torch.Tensor) -> torch.Tensor:
        # x: (B,T,H,D)
        T = x_BTHD.size(-3)
        cos = self._cos[None, :T, None, :]
        sin = self._sin[None, :T, None, :]
        return apply_rotary_emb(x_BTHD, cos, sin)

    def reset(self):
        # Restore base (unscaled) frequencies and attention scale.
        self.mrope.reset()
        self.attn_scale = 0.1

    def apply(self, old_window: int, new_window: int, alpha: int = 1, beta: int = 32):
        self.mrope.apply(old_window, new_window, alpha=alpha, beta=beta)
        self.attn_scale *= 0.2 * math.log(float(new_window) / float(old_window)) + 1.0


class YarnPairedHead(nn.Module):
    """YaRN + 4-axis Interleaved-MRoPE for paired-head attention layers."""
    ...
    def __init__(self, head_dim: int, max_seq_len: int):
        ...
        # Per-forward cos/sin buffers (even/odd positions in the interleaved 2T sequence).
        self._cos_even = nn.Buffer(
            torch.empty(self.max_seq_len, self.mrope.total_pairs, device=device, dtype=torch.bfloat16),
            persistent=False,
        )
        self._sin_even = nn.Buffer(
            torch.empty(self.max_seq_len, self.mrope.total_pairs, device=device, dtype=torch.bfloat16),
            persistent=False,
        )
        self._cos_odd = nn.Buffer(
            torch.empty(self.max_seq_len, self.mrope.total_pairs, device=device, dtype=torch.bfloat16),
            persistent=False,
        )
        self._sin_odd = nn.Buffer(
            torch.empty(self.max_seq_len, self.mrope.total_pairs, device=device, dtype=torch.bfloat16),
            persistent=False,
        )
    ...
    def set_token_ids(self, token_ids: torch.Tensor) -> None:
        cos_e, sin_e, cos_o, sin_o = self.mrope.build_cos_sin_even_odd(token_ids)
        T = cos_e.size(1)
        self._cos_even[:T].copy_(cos_e[0])
        self._sin_even[:T].copy_(sin_e[0])
        self._cos_odd[:T].copy_(cos_o[0])
        self._sin_odd[:T].copy_(sin_o[0])

    def rotary(self, x_BTHD: torch.Tensor) -> torch.Tensor:
        # x: (B,T,H,2D) where the last dim concatenates two heads.
        T = x_BTHD.size(-3)
        D = self.head_dim
        x_a = x_BTHD[..., :D]
        x_b = x_BTHD[..., D:]

        cos_e = self._cos_even[None, :T, None, :]
        sin_e = self._sin_even[None, :T, None, :]
        cos_o = self._cos_odd[None, :T, None, :]
        sin_o = self._sin_odd[None, :T, None, :]

        y_a = apply_rotary_emb(x_a, cos_e, sin_e)
        y_b = apply_rotary_emb(x_b, cos_o, sin_o)
...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, seqlens: Tensor, schedule_cfg: ForwardScheduleConfig):
        ...
        # Precompute 4-axis Interleaved-MRoPE cos/sin once per forward (reused across layers)
        self.yarn.set_token_ids(input_seq)
        self.yarn_paired_head.set_token_ids(input_seq)
