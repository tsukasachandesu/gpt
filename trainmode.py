...
# -----------------------------------------------------------------------------
# FlexAttention BlockMask helpers (computed outside torch.compile)

def _build_flex_block_mask(cu_seqlens: Tensor, total_len: int, window: int) -> object:
    """
    Build a BlockMask equivalent to flash_attn_varlen_func(causal=True, window_size=(window,0))
    plus document masking for packed sequences defined by cu_seqlens.

    Notes:
      * Must be called OUTSIDE any torch.compile(fullgraph=True) region.
      * We use B=None/H=None to broadcast across batch/heads (mask is head-independent).
    """
    # Map each token position -> document id
    # cu_seqlens is non-decreasing, last element is total_len (often repeated for padding).
    pos = torch.arange(total_len, device=cu_seqlens.device, dtype=cu_seqlens.dtype)
    ...
    def mask_mod(b: Tensor, h: Tensor, q_idx: Tensor, kv_idx: Tensor) -> Tensor:
        ...
        return same_doc & causal & window_mask
...
class TrainingManager():
    ...
    def get_forward_args(self, cum_seqlens: Tensor, seq_len: int):
        ...
        bm_short = _build_flex_block_mask(cum_seqlens, seq_len, short_bm)
        bm_long = _build_flex_block_mask(cum_seqlens, seq_len, long_bm)

        # paired-head masks operate on the interleaved sequence (length *2)
        cum_seqlens_p = cum_seqlens * 2
        seq_len_p = seq_len * 2
        bm_short_paired = _build_flex_block_mask(cum_seqlens_p, seq_len_p, short_bm)
        bm_long_paired = _build_flex_block_mask(cum_seqlens_p, seq_len_p, long_bm)
